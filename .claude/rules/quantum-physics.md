...
| Riemann Zeta (Arithmetic Horizon) | `riemannZeta` | 3D-11D | ζ-zero spectral synthesis ⇄ primon gas on a log-radial LUT (explicit formula) | source (zeros/primes), Nz, β (Hagedorn at β=1), horizon r_h, ℓ, m, dilation flow, glow, cutaway; GUE spacing panel in Analysis tab |
| Hilbert–Pólya Spectrum | `hilbertPolya` | 3D | Evans-landscape filament volume (Re z, Im z, θ) of the Riemann operator R̂ = −D̂ − iμ(T̂) | zMax, yExtent, filamentWidth, glow, fogGain, planeMarker; zeros pinned to Im z = 0; eta comb at Im z = −1/2; θ-axis lifts the Matsubara veil; Weil-positivity monitor in Analysis tab |
