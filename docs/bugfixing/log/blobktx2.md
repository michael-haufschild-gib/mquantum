...
self.addEventListener("message", function(g) {
    ...
    case "init":
        s = w.config,
        n(w.transcoderBinary);
        break;
    case "transcode":
        e.then( () => {
            try {
                const {faces: y, buffers: b, width: S, height: x, hasAlpha: v, format: C, type: M, dfdFlags: I} = l(w.buffer);
                self.postMessage({
                    type: "transcode",
                    id: w.id,
                    data: {
...
c(n, "init");
function l(g) {
    const w = new t.KTX2File(new Uint8Array(g));
    function y() {
        w.close(),
        w.delete()
    }
    ...
    const S = w.getWidth()
      ...
      , I = w.getHasAlpha()
      , T = w.getDFDFlags()
      , {transcoderFormat: D, engineFormat: P, engineType: E} = p(b, S, x, I);
    if (!S || !x || !C)
        throw y(),
        new Error("THREE.KTX2Loader:	Invalid texture");
    if (!w.startTranscoding())
        throw y(),
    ...
    for (let k = 0; k < M; k++) {
        ...
        for (let z = 0; z < C; z++) {
            ...
            let Q, H;
            for (let U = 0; U < v; U++) {
                const _ = w.getImageLevelInfo(z, U, k);
                k === 0 && z === 0 && U === 0 && (_.origWidth % 4 !== 0 || _.origHeight % 4 !== 0) && console.warn("THREE.KTX2Loader: ETC1S and UASTC textures should use multiple-of-four dimensions."),
                C > 1 ? (Q = _.origWidth,
                H = _.origHeight) : (Q = _.width,
                H = _.height);
                let Y = new Uint8Array(w.getImageTranscodedSizeInBytes(z, U, 0, D));
                const R = w.transcodeImage(Y, z, U, k, D, 0, -1, -1);
                if (E === i.HalfFloatType && (Y = new Uint16Array(Y.buffer,Y.byteOffset,Y.byteLength / Uint16Array.BYTES_PER_ELEMENT)),
                !R)
                    throw y(),
...
const d = [{
    ...
    engineFormat: [o.RGBA_ASTC_4x4_Format, o.RGBA_ASTC_4x4_Format],
    ...
    engineFormat: [o.RGBA_S3TC_DXT1_Format, o.RGBA_S3TC_DXT5_Format],
    engineType: [i.UnsignedByteType],
    priorityETC1S: 4,
    priorityUASTC: 5,
    needsPowerOfTwo: !1
    ...
    engineFormat: [o.RGB_ETC1_Format],
    engineType: [i.UnsignedByteType],
    priorityETC1S: 2,
    priorityUASTC: 4,
    needsPowerOfTwo: !1
    ...
    engineFormat: [o.RGB_PVRTC_4BPPV1_Format, o.RGBA_PVRTC_4BPPV1_Format],
    engineType: [i.UnsignedByteType],
    priorityETC1S: 5,
    priorityUASTC: 6,
    needsPowerOfTwo: !0
    ...
    engineFormat: [o.RGB_BPTC_UNSIGNED_Format],
    ...
    engineFormat: [o.RGBAFormat, o.RGBAFormat],
    ...
    engineFormat: [o.RGBAFormat],
    engineType: [i.HalfFloatType],
    priorityHDR: 100,
    needsPowerOfTwo: !1
}]
  , h = {
    [a.ETC1S]: d.filter(g => g.basisFormat.includes(a.ETC1S)).sort( (g, w) => g.priorityETC1S - w.priorityETC1S),
    [a.UASTC]: d.filter(g => g.basisFormat.includes(a.UASTC)).sort( (g, w) => g.priorityUASTC - w.priorityUASTC),
    [a.UASTC_HDR]: d.filter(g => g.basisFormat.includes(a.UASTC_HDR)).sort( (g, w) => g.priorityHDR - w.priorityHDR)
};
function p(g, w, y, b) {
    const S = h[g];
    for (let x = 0; x < S.length; x++) {
        const v = S[x];
        if (v.if && !s[v.if] || !v.basisFormat.includes(g) || b && v.transcoderFormat.length < 2 || v.needsPowerOfTwo && !(m(w) && m(y)))
            continue;
        const C = v.transcoderFormat[b ? 1 : 0]
          , M = v.engineFormat[b ? 1 : 0]
          , I = v.engineType[0];
        return {
            transcoderFormat: C,
            engineFormat: M,
