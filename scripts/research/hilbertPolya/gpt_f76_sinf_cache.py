...
def residual(curvature: list[mp.mpf], j: int) -> mp.mpf:
    return curvature[j] - c_w(j)
...
def main() -> None:
    ...
    for k in requested:
        s_k = mp.fsum(residual(curvature, j) for j in range(1, k + 1))
        partials[k] = mp.nstr(s_k, 50)

    c0_rows: dict[int, dict[str, str]] = {}
    c0_cutoffs = sorted({40, 80, 120, 140, 160, 166, min(max_valid, 169), *requested})
    for k in c0_cutoffs:
        if k > max_valid:
            continue
    ...
    residual_blocks: dict[str, dict[str, str]] = {}
    previous = 0
    for k in requested:
        values = [residual(curvature, j) for j in range(previous + 1, k + 1)]
        residual_blocks[f"{previous + 1}-{k}"] = {
            "sum": mp.nstr(mp.fsum(values), 40),
            "min": mp.nstr(min(values), 30),
            "max": mp.nstr(max(values), 30),
            "last": mp.nstr(values[-1], 30),
        }
        previous = k
    ...
    result = {
        ...
        "interpretation": (
            "Cache partials test the one-term W-telescoping constant. Agreement "
            "with the pre-registered e scale is a finite-cutoff diagnostic only; "
            "a crossing or persistent same-sign residual tail means the next "
            f"asymptotic correction is still part of the proof debt (curvature "
            f"extends through {max_valid})."
        ),
    }
