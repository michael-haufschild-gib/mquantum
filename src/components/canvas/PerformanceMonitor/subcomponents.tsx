...
export const InfoCard = ({
  ...
  <div className="bg-[var(--bg-hover)] rounded-md p-2 border border-[var(--border-subtle)]">
    ...
    <div
      className={`font-mono text-xs ${highlight ? 'text-accent font-bold' : 'text-[var(--text-secondary)]'}`}
    >
      {value}
    </div>
  </div>
...
export const Sparkline = React.memo(function Sparkline({
  ...
  return (
    <svg width={width} height={height} className="overflow-visible">
      {fill && <path d={fillD} fill={color} fillOpacity={0.1} stroke="none" />}
      <path
        d={pathD}
        fill="none"
        stroke={color}
