...
export const EditorTopBar: React.FC<EditorTopBarProps> = React.memo(({
  ...
  const handleExportVideo = useCallback(async () => {
    ...
    if (objectType === 'schroedinger') {
      const modeName =
        quantumMode === 'harmonicOscillator' ? 'Harmonic Oscillator' : 'Hydrogen Orbitals';
  ...
  const onConfirmSaveStyle = useCallback((name: string) => {
  ...
  const onConfirmSaveScene = useCallback((name: string) => {
    const trimmedName = name.trim();
    if (trimmedName) {
