          ...
          {!isRunning && (
            <div className="grid grid-cols-3 gap-1.5">
              <Tooltip content="Lower bound of the coupling strength range (log-spaced). Controls the integrability-to-chaos transition.">
                <div>
                  <NumberInput
                    data-testid="components-sections-analysis-quantumness-atlas-section-number-input-219-19"
                    label="λ min"
                    value={lambdaMin}
              ...
              <Tooltip content="Upper bound of the coupling strength range (log-spaced).">
                <div>
                  <NumberInput
                    data-testid="components-sections-analysis-quantumness-atlas-section-number-input-231-19"
                    label="λ max"
                    value={lambdaMax}
              ...
              <Tooltip content="Number of log-spaced λ values to sweep. More steps = finer resolution but longer runtime.">
                <div>
                  <NumberInput
                    data-testid="components-sections-analysis-quantumness-atlas-section-number-input-243-19"
                    label="λ steps"
                    value={lambdaSteps}
          ...
          <div className="flex items-center gap-1.5">
            {!isRunning ? (
              <Tooltip content="Begin the 3-axis parameter sweep (N × λ × γ). Automatically configures TDSE potential, monitoring, and entanglement diagnostics for each point. Physics state is restored after sweep completes or is aborted.">
                <div>
                  <Button
                    data-testid="components-sections-analysis-quantumness-atlas-section-button-261-19"
                    variant="primary"
                    size="sm"
                    onClick={handleStart}
              ...
              <Tooltip content="Stop the sweep and restore pre-sweep physics state. Completed points are preserved.">
                <div>
                  <Button
                    data-testid="components-sections-analysis-quantumness-atlas-section-button-274-19"
                    variant="secondary"
                    size="sm"
                    onClick={handleAbortAtlasSweep}
                  >
                    Abort
                  </Button>
                </div>
          ...
          {results.length > 0 && (
            <>
              <Tooltip content="Choose how to visualize the atlas data. Erosion curves show how diagnostics decay with monitoring rate γ. Scatter plots reveal correlations between measures.">
                <div>
                  <Select
                    data-testid="components-sections-analysis-quantumness-atlas-section-select-312-19"
                    label="View"
                    value={view}
                ...
                <div className="space-y-1.5">
                  <div className="grid grid-cols-2 gap-1.5">
                    {availableLambdas.length > 1 && (
                      <Select
                        data-testid="components-sections-analysis-quantumness-atlas-section-select-338-23"
                        label="λ"
                    ...
                    {availableDims.length > 1 && (
                      <Select
                        data-testid="components-sections-analysis-quantumness-atlas-section-select-350-23"
                        label="N"
              ...
              {view === 'heatmap' && (
                <div className="space-y-1.5">
                  {availableGammas.length > 1 && (
                    <Select
                      data-testid="components-sections-analysis-quantumness-atlas-section-select-370-21"
                      label="γ"
              ...
              {view === 'dimCompare' && (
                <div className="space-y-1.5">
                  <div className="grid grid-cols-2 gap-1.5">
                    {availableLambdas.length > 1 && (
                      <Select
                        data-testid="components-sections-analysis-quantumness-atlas-section-select-390-23"
                        label="λ"
                        value={String(activeLambda)}
                        onChange={(v) => setSelectedLambda(Number(v))}
                        options={availableLambdas.map((l) => ({
                    ...
                    {availableGammas.length > 1 && (
                      <Select
                        data-testid="components-sections-analysis-quantumness-atlas-section-select-402-23"
                        label="γ"
                        value={String(activeGamma)}
                        onChange={(v) => setSelectedGamma(Number(v))}
                        options={availableGammas.map((g) => ({
              ...
              <div className="flex gap-1.5 pt-1 border-t border-border-subtle">
                <Tooltip content="Download atlas results as CSV with columns: dim, lambda, gamma, avg_normalized_entropy, variance, Wigner negativity, IPR, grid size, sample counts.">
                  <div>
                    <Button
                      data-testid="components-sections-analysis-quantumness-atlas-section-button-422-21"
                      variant="ghost"
                      size="sm"
                ...
                <Tooltip content="Download atlas results as JSON with full numeric precision.">
                  <div>
                    <Button
                      data-testid="components-sections-analysis-quantumness-atlas-section-button-429-21"
                      variant="ghost"
                      size="sm"
