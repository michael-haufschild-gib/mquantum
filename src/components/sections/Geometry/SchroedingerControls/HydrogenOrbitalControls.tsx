...
export const HydrogenOrbitalControls: React.FC<HydrogenOrbitalControlsProps> = React.memo(({
  ...
  const presetOptions = useMemo(() => {
    ...
    return [
      { label: 's Orbitals (Spherical)', options: groups.s.map(([k, p]) => ({ value: k, label: p.name })) },
      { label: 'p Orbitals (Dumbbell)', options: groups.p.map(([k, p]) => ({ value: k, label: p.name })) },
      { label: 'd Orbitals (Cloverleaf)', options: groups.d.map(([k, p]) => ({ value: k, label: p.name })) },
      { label: 'f Orbitals (Complex)', options: groups.f.map(([k, p]) => ({ value: k, label: p.name })) },
