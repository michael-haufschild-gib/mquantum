 ...
 */
const SHADOW_QUALITY_SELECT_OPTIONS: SelectOption<ShadowQuality>[] = SHADOW_QUALITY_OPTIONS.map(
...
export const ShadowsSection: React.FC<ShadowsSectionProps> = React.memo(
  ({ defaultOpen = false }) => {
    ...
    const handleShadowStepsChange = useCallback(
      (val: string) => {
        ...
        {hasEnabledLights ? (
          <div className="space-y-4">
            ...
            <div className="px-2 py-1.5 rounded bg-[var(--bg-hover)] border border-border-default">
              <p className="text-[10px] font-medium text-text-primary">{shadowTypeInfo.label}</p>
              <p className="text-[10px] text-text-secondary">{shadowTypeInfo.description}</p>
            </div>
          ...
          <div className="p-4 rounded-lg bg-[var(--bg-hover)] border border-border-subtle border-dashed text-center">
            <p className="text-xs text-text-secondary italic">Add lights to enable shadows.</p>
          </div>
        )}
      </Section>
    )
