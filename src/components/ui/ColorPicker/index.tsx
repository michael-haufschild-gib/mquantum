...
interface ColorPickerProps {
  ...
  tooltip?: string
  /** Stable selector for the color picker trigger and popover controls. */
  'data-testid'?: string
}
          ...
          content={
            <div className="w-[260px] p-3 flex flex-col gap-3 select-none text-text-primary">
                ...
                <div className="flex items-center gap-1">
                  ...
                  {typeof window !== 'undefined' && 'EyeDropper' in window && (
                    <button
                      ...
                      data-testid={dataTestId ? `${dataTestId}-eyedropper` : undefined}
                      title="Pick color"
                    >
                      <EyeDropperIcon />
                    </button>
              ...
              <div
                ...
                data-testid={dataTestId ? `${dataTestId}-saturation-brightness` : undefined}
              ...
              <div className="space-y-3">
                ...
                {!disableAlpha && (
                  <div className="h-3 rounded-full relative overflow-hidden ring-1 ring-border-default cursor-pointer group">
                    ...
                    />
                    <input
                      data-testid={dataTestId ? `${dataTestId}-opacity` : undefined}
                      type="range"
                      min={0}
                      max={1}
                      step={0.01}
                ...
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <div className="flex bg-[var(--bg-active)] rounded p-0.5">
                    {(['HEX', 'RGB'] as const).map((m) => (
                      <button
                        data-testid={
                          dataTestId ? `${dataTestId}-mode-${m.toLowerCase()}` : undefined
                        }
                        key={m}
                        type="button"
                        onClick={() => setMode(m)}
                  ...
                  <div className="flex gap-2">
                    ...
                    {!disableAlpha && (
                      <div className="w-14 bg-[var(--bg-hover)] border border-border-default rounded px-1 py-1 flex items-center gap-1 group-focus-within:border-accent/50 transition-colors">
                        ...
                        <input
                          data-testid={dataTestId ? `${dataTestId}-opacity-percent` : undefined}
                          type="number"
                          min={0}
                          max={100}
                  ...
                  <div className="flex gap-1.5">
                    {(['r', 'g', 'b'] as const).map((c) => (
                      ...
                      >
                        ...
                        <input
                          data-testid={dataTestId ? `${dataTestId}-rgb-${c}` : undefined}
                          type="number"
                          min={0}
                          max={255}
                          value={rgbInput[c]}
              ...
              <div className="space-y-2 pt-1">
                ...
                <div className="flex gap-1 justify-between">
                  {palette.map((c, i) => (
                    <button
                      data-testid={dataTestId ? `${dataTestId}-palette-${i}` : undefined}
                      key={i}
                      type="button"
                ...
                {history.length > 0 && (
                  <div className="flex gap-1.5 flex-wrap pt-2 border-t border-border-subtle">
                    {history.map((c, i) => (
                      <button
                        data-testid={dataTestId ? `${dataTestId}-history-${i}` : undefined}
                        key={i}
                        type="button"
                        onClick={() => handleColorSelection(c)}
