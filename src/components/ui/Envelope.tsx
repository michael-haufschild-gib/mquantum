...
export const Envelope: React.FC<EnvelopeProps> = React.memo(
  ({
    ...
    const pRelease =
      mode === 'AR' || mode === 'ADSR' || mode === 'AHDSR'
        ? Math.max(0.01, release)
        : mode === 'AD'
          ? Math.max(0.01, decay)
          : 0 // For AD, decay is effectively release from peak
    ...
    const points = useMemo(
      () => [
        ...
        {
          x: xDecay,
          y: mode === 'AD' || mode === 'AR' ? yBottom : ySustain,
          label: 'decay',
          hidden: mode === 'AD' || mode === 'AR',
        },
