...
export function ProdDevDiagnostics() {
  ...
  useEffect(() => {
    const logDiagnostics = () => {
      ...
      const avgRenderCalls =
        renderCounts.length > 0 ? renderCounts.reduce((a, b) => a + b, 0) / renderCounts.length : 0
      ...
      const fragPrecision = context.getShaderPrecisionFormat(
        context.FRAGMENT_SHADER,
        context.HIGH_FLOAT
      )
      const vertPrecision = context.getShaderPrecisionFormat(
        context.VERTEX_SHADER,
        context.HIGH_FLOAT
      )
      const fragmentPrecision = fragPrecision
        ? `${fragPrecision.precision}bit (${fragPrecision.rangeMin}-${fragPrecision.rangeMax})`
        : 'unknown'
      const vertexPrecision = vertPrecision
        ? `${vertPrecision.precision}bit (${vertPrecision.rangeMin}-${vertPrecision.rangeMax})`
        : 'unknown'
      ...
      console.log(
        '%cPrograms/Geometries/Textures:',
        'font-weight: bold',
        `${data.programCount} / ${data.geometryCount} / ${data.textureCount}`
      )
      console.log(
        '%cShader Precision (frag/vert):',
        'font-weight: bold',
        `${data.fragmentPrecision} / ${data.vertexPrecision}`
      )
      console.log('%cExtensions:', 'font-weight: bold', data.extensionCount)
      console.log(
        '%cViewport:',
        'font-weight: bold',
        `${data.cssWidth}x${data.cssHeight} CSS, ${data.nativeWidth}x${data.nativeHeight} native (DPR: ${data.dpr})`
      )
      console.log(
        '%cRender Scale:',
        'font-weight: bold',
        `${data.renderResolutionScale} (${data.scaledWidth}x${data.scaledHeight} = ${data.scaledPixels.toLocaleString()} scaled pixels)`
      )
      ...
      console.log(
        '%cAvg Frame Time:',
        'font-weight: bold',
        `${data.avgFrameTime.toFixed(2)}ms (${(1000 / data.avgFrameTime).toFixed(1)} FPS)`
      )
