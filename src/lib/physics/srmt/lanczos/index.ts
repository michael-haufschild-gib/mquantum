...
const MAX_LANCZOS_DENSE_ENTRIES = 2 ** 20
const MAX_LANCZOS_BASIS_ENTRIES = 2 ** 20

function resolveDenseMatrixSize(caller: string, n: number): number {
  if (!Number.isInteger(n) || n < 0) {
    throw new RangeError(`${caller}: n must be a non-negative integer, got ${n}`)
  }
  if (n > Math.floor(Math.sqrt(MAX_LANCZOS_DENSE_ENTRIES))) {
    throw new RangeError(`${caller}: n² exceeds ${MAX_LANCZOS_DENSE_ENTRIES} entries, got n=${n}`)
  }
  return n * n
}

function resolveBasisEntries(caller: string, maxIters: number, n: number): number {
  if (maxIters > Math.floor(MAX_LANCZOS_BASIS_ENTRIES / Math.max(1, n))) {
    throw new RangeError(
      `${caller}: Krylov basis size exceeds ${MAX_LANCZOS_BASIS_ENTRIES} entries`
    )
  }
  return maxIters * n
}

function assertFiniteMatrix(matrix: Float32Array, size: number): void {
  for (let i = 0; i < size; i++) {
    if (!Number.isFinite(matrix[i]!)) {
      throw new Error(`lanczosTopK: non-finite matrix entry at index ${i}`)
    }
  }
}

function assertFiniteVector(vec: Float64Array, label: string): void {
  for (let i = 0; i < vec.length; i++) {
    if (!Number.isFinite(vec[i]!)) {
      throw new Error(`${label}: non-finite operator output at index ${i}`)
    }
  }
}
...
function randomUnitVector(vec: Float64Array, seed: number): void {
  ...
  const inv = 1 / Math.max(norm, Number.MIN_VALUE)
...
export function lanczosTopK(
  ...
  const matrixSize = resolveDenseMatrixSize('lanczosTopK', n)
  if (matrix.length < matrixSize) {
    throw new Error(`lanczosTopK: buffer length ${matrix.length} < n² = ${matrixSize}`)
  }
  assertFiniteMatrix(matrix, matrixSize)
...
export function lanczosTopKOp(
  ...
  if (n > MAX_LANCZOS_BASIS_ENTRIES) {
    throw new RangeError(`lanczosTopKOp: n exceeds max supported basis entries, got ${n}`)
  }
  ...
  const basisEntries = resolveBasisEntries('lanczosTopKOp', maxIters, n)
  const Q = new Float64Array(basisEntries)
  ...
  for (let j = 0; j < maxIters; j++) {
    // w ← A · q_j
    applyA(qCurr, w)
    assertFiniteVector(w, 'lanczosTopKOp')
    ...
    // β_j ← ||w||
    let b = 0
    for (let i = 0; i < n; i++) b = Math.hypot(b, w[i]!)
    beta[j] = b
