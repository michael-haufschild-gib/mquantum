...
import { complexSvdSingularValues, MAX_COMPLEX_SVD_MATRIX_ENTRIES } from '../svd'
...
function resolveChiTensorShape(tensor: ChiTensor): {
  Na: number
  Nphi1: number
  Nphi2: number
  phiSq: number
  ...
  let totalSites = 1
  for (let d = 0; d < 3; d++) {
    const size = tensor.gridSize[d]!
    if (!Number.isSafeInteger(size) || size <= 0) {
      throw new RangeError(
        `reshapeForClock: gridSize[${d}] must be a positive safe integer, got ${size}`
      )
    }
    if (totalSites > Math.floor(Number.MAX_SAFE_INTEGER / size)) {
      throw new RangeError('reshapeForClock: grid product exceeds safe integer range')
    }
    if (totalSites > Math.floor(MAX_COMPLEX_SVD_MATRIX_ENTRIES / size)) {
      throw new RangeError(
        `reshapeForClock: Schmidt matrix entries exceed ${MAX_COMPLEX_SVD_MATRIX_ENTRIES}`
      )
    }
    totalSites *= size
  }
  const [Na, Nphi1, Nphi2] = tensor.gridSize
  return { Na, Nphi1, Nphi2, phiSq: Nphi1 * Nphi2, totalSites }
}
...
export function reshapeForClock(tensor: ChiTensor, clock: SrmtClock): ComplexMatrix {
  const { Na, Nphi1, Nphi2, phiSq, totalSites } = resolveChiTensorShape(tensor)
  const chi = tensor.chi

  if (chi.length !== 2 * totalSites) {
    throw new Error(
