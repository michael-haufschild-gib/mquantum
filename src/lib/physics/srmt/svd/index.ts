...
export const MAX_COMPLEX_SVD_MATRIX_ENTRIES = 2 ** 20
export const MAX_COMPLEX_SVD_REAL_EMBEDDING_ENTRIES = 2 ** 20

function resolveSvdShape(M: ComplexMatrix): { entries: number; kExpected: number } {
  const { rows: m, cols: n } = M
  if (m < 0 || n < 0 || !Number.isSafeInteger(m) || !Number.isSafeInteger(n)) {
    throw new Error(`complexSvdSingularValues: invalid shape (${m}, ${n})`)
  }
  if (m > 0 && n > Math.floor(Number.MAX_SAFE_INTEGER / m)) {
    throw new Error(`complexSvdSingularValues: matrix entry count overflows safe integer range`)
  }
  const entries = m * n
  if (entries > MAX_COMPLEX_SVD_MATRIX_ENTRIES) {
    throw new Error(
      `complexSvdSingularValues: matrix entries ${entries} exceed ${MAX_COMPLEX_SVD_MATRIX_ENTRIES}`
    )
  }
  const kExpected = Math.min(m, n)
  const realEmbeddingOrder = 2 * kExpected
  if (
    realEmbeddingOrder > 0 &&
    realEmbeddingOrder > Math.floor(Math.sqrt(MAX_COMPLEX_SVD_REAL_EMBEDDING_ENTRIES))
  ) {
    throw new Error(
      `complexSvdSingularValues: real embedding exceeds ${MAX_COMPLEX_SVD_REAL_EMBEDDING_ENTRIES} entries`
    )
  }
  return { entries, kExpected }
}
...
export function complexSvdSingularValues(M: ComplexMatrix): Float64Array {
  const { rows: m, cols: n, re, im } = M
  const { entries, kExpected } = resolveSvdShape(M)
  if (re.length !== entries || im.length !== entries) {
    throw new Error(
      `complexSvdSingularValues: buffer length !== m·n (re=${re.length}, im=${im.length}, m·n=${entries})`
    )
  }
