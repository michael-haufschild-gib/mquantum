...
export const TDSE_SCENARIO_PRESETS: TdseScenarioPreset[] = [
  ...
  },
  {
    ...
    name: 'Postselected CTC: Novikov Loop',
    description:
      'Mirror-pair P-CTC filter with zero loop phase. The paradox sector is damped until the wavefunction satisfies the Novikov fixed-point relation psi(v)=psi(M(v)), visibly cloning the left packet into a loop-consistent two-mouth standing structure.',
    maxDim: 3,
    overrides: {
      latticeDim: 3,
      gridSize: [64, 64, 64],
      ...
      stepsPerFrame: 4,
      initialCondition: 'gaussianPacket',
      packetCenter: [-1.15, 0, 0],
      packetWidth: 0.34,
      packetAmplitude: 1.0,
      packetMomentum: [0, 0, 0],
      potentialType: 'doubleWell',
      doubleWellLambda: 8.0,
      doubleWellSeparation: 1.15,
      doubleWellAsymmetry: 0.0,
      absorberEnabled: true,
      absorberWidth: 0.2,
      pmlTargetReflection: 1e-6,
      ...
      ctcPostselectionStrength: 1.0,
      ctcLoopPhase: 0,
      wormholeCoherenceHudEnabled: false,
    },
    renderingOverrides: { densityGain: 4.0, densityContrast: 2.3, autoScaleMaxGain: 20 },
  },
  {
    id: 'postselectedCtcParadoxGate',
    name: 'Postselected CTC: Paradox Gate',
    description:
      'Mirror-pair P-CTC filter with pi loop holonomy. The accepted history is antisymmetric across the mouths, so the density remains paired while phase view exposes the minus-sign paradox gate carved into the loop.',
    maxDim: 3,
    overrides: {
      latticeDim: 3,
      gridSize: [64, 64, 64],
  ...
  },
  {
    ...
    name: 'CTC Residue: Novikov Map',
    description:
      'Postselected CTC residue field for a weakly filtered mirror-mouth superposition with zero loop holonomy. Bright sheets are histories still being rejected before the Novikov fixed point closes.',
    maxDim: 3,
    overrides: {
      latticeDim: 3,
      gridSize: [64, 64, 64],
      ...
      wormholeMirrorAxis: 0,
      ctcPostselectionEnabled: true,
      ctcPostselectionStrength: 0.006,
      ctcLoopPhase: 0,
      wormholeCoherenceHudEnabled: false,
    },
    renderingOverrides: { densityGain: 5.0, densityContrast: 3.0, autoScaleMaxGain: 60 },
  },
  {
    id: 'ctcResidualParadoxMap',
    name: 'CTC Residue: Paradox Map',
    description:
      'Same weak P-CTC residue geometry with pi loop holonomy. The accepted mirror echo is phase-flipped, so the residue map lights a different paradox sector than the Novikov map.',
    maxDim: 3,
    overrides: {
      latticeDim: 3,
      gridSize: [64, 64, 64],
  ...
  },
  {
    ...
    name: 'CTC Loop Gain: Constructive Horizon',
    description:
      'Chronology-horizon loop-gain view for a symmetric mirror-mouth superposition with zero holonomy. Bright regions mark mirror pairs whose phases close into resonant repeated-loop amplification.',
    maxDim: 3,
    overrides: {
      latticeDim: 3,
      gridSize: [64, 64, 64],
    ...
    renderingOverrides: { densityGain: 7.0, densityContrast: 3.4, autoScaleMaxGain: 80 },
  },
  {
    id: 'ctcLoopGainShearedProtection',
    name: 'CTC Loop Gain: Sheared Protection',
    description:
      'Same mirror-mouth geometry with pi-over-two loop holonomy and transverse phase shear. Resonant bands collapse where the mirror echo arrives out of phase, showing destructive chronology protection.',
    maxDim: 3,
    overrides: {
      latticeDim: 3,
      gridSize: [64, 64, 64],
      ...
      ctcPostselectionEnabled: false,
      ctcPostselectionStrength: 0.97,
      ctcLoopPhase: Math.PI / 2,
      wormholeCoherenceHudEnabled: false,
    },
    renderingOverrides: { densityGain: 9.0, densityContrast: 3.8, autoScaleMaxGain: 100 },
  },
  {
    id: 'ctcDeutschEntropyParadoxMixer',
    name: 'Deutsch Entropy: Paradox Mixer',
    description:
      'Deutsch fixed-point entropy view for a balanced mirror-mouth superposition with pi loop holonomy. Equal-amplitude mirror histories contradict the echo, so the entropy scalar burns bright where the loop has no pure-history resolution.',
    maxDim: 3,
    overrides: {
      latticeDim: 3,
      gridSize: [64, 64, 64],
    ...
    renderingOverrides: { densityGain: 9.0, densityContrast: 4.0, autoScaleMaxGain: 100 },
  },
  {
    id: 'ctcDeutschEntropyShearedMixer',
    name: 'Deutsch Entropy: Sheared Mixer',
    description:
      'Same Deutsch entropy mirror-mouth geometry with pi-over-two loop holonomy and transverse momentum shear. The entropy remains nonzero but breaks into sheared bands, separating partial paradox from full contradiction.',
    maxDim: 3,
    overrides: {
      latticeDim: 3,
      gridSize: [64, 64, 64],
