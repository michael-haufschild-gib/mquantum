...
/**
 * Resolve and validate the WdW density texture edge length before allocating
 * packed RGBA buffers or CPU scratch.
 *
 * @param targetGridSize - Requested density texture edge length.
 * @returns Rounded, validated edge length.
 */
export function resolveWdwDensityGridSize(targetGridSize: number = DENSITY_GRID_SIZE): number {
  if (!Number.isFinite(targetGridSize)) {
    throw new Error(`packWdwDensityGrid: targetGridSize must be finite, got ${targetGridSize}`)
  }
  const N = Math.max(1, Math.round(targetGridSize))
  if (!Number.isSafeInteger(N) || N > MAX_WDW_DENSITY_GRID_SIZE) {
    throw new Error(
      `packWdwDensityGrid: targetGridSize ${N} exceeds max supported size ${MAX_WDW_DENSITY_GRID_SIZE}`
    )
  }
  return N
}
