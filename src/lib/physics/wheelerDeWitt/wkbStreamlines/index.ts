...
export const MAX_WKB_STREAMLINE_OVERLAY_SITES = 2 ** 20
...
function resolveWkbGridSites(gridSize: [number, number, number], caller: string): number {
  let total = 1
  for (let d = 0; d < 3; d++) {
    const size = gridSize[d]!
    if (!Number.isSafeInteger(size) || size <= 0) {
      throw new RangeError(`${caller}: gridSize[${d}] must be a positive safe integer, got ${size}`)
    }
    if (total > Math.floor(Number.MAX_SAFE_INTEGER / size)) {
      throw new RangeError(`${caller}: grid product exceeds safe integer range`)
    }
    if (total > Math.floor(MAX_WKB_STREAMLINE_OVERLAY_SITES / size)) {
      throw new RangeError(
        `${caller}: grid product exceeds site budget ${MAX_WKB_STREAMLINE_OVERLAY_SITES}`
      )
    }
    total *= size
  }
...
function buildArgTable(output: WheelerDeWittSolverOutput): ArgTable {
  ...
  const total = resolveWkbGridSites(output.gridSize, 'integrateWkbTrajectories')
  if (output.chi.length < total * 2) {
    throw new RangeError(
      `integrateWkbTrajectories: chi buffer too small (need ${total * 2}, got ${output.chi.length})`
    )
  }
...
export function integrateWkbTrajectories(
  ...
  const [Na, Nphi] = output.gridSize
  const total = resolveWkbGridSites(output.gridSize, 'integrateWkbTrajectories')
  if (output.lorentzianMask.length < total) {
    throw new RangeError(
      `integrateWkbTrajectories: lorentzianMask buffer too small (need ${total}, got ${output.lorentzianMask.length})`
    )
  }
...
export function buildStaticOverlay(
  ...
  const total = resolveWkbGridSites(gridSize, 'buildStaticOverlay')
...
export function buildPulseOverlay(
  ...
  const total = resolveWkbGridSites(gridSize, 'buildPulseOverlay')
  const intensity = pickOverlayBuffer(out, total)
