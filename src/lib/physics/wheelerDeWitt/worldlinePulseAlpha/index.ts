...
const MAX_WDW_PULSE_TARGET_GRID_SIZE = 256

function resolvePulseTargetGridSize(targetGridSize: number): number {
  if (!Number.isFinite(targetGridSize)) {
    throw new Error(`applyWdwPulseAlphaRows: targetGridSize must be finite, got ${targetGridSize}`)
  }
  const N = Math.max(1, Math.round(targetGridSize))
  if (!Number.isSafeInteger(N) || N > MAX_WDW_PULSE_TARGET_GRID_SIZE) {
    throw new Error(
      `applyWdwPulseAlphaRows: targetGridSize ${N} exceeds max supported size ${MAX_WDW_PULSE_TARGET_GRID_SIZE}`
    )
  }
  return N
}
...
function ensurePulseSampleTables(
  ...
  targetGridSize = resolvePulseTargetGridSize(targetGridSize)
...
export function applyWdwPulseAlphaRows(
  ...
  const N = resolvePulseTargetGridSize(targetGridSize)
  const totalRows = N * N
