...
export function VideoExportController() {
  ...
  const originalPerfSettingsRef = useRef<{
    quality: number
    lowQualityAnim: boolean
  ...
  const restoreState = useCallback(() => {
    ...
    perfStore.setProgressiveRefinementEnabled(
      originalPerfSettingsRef.current.progressiveRefinementEnabled
    )
  ...
  const processBatch = useCallback(async () => {
    ...
    try {
      ...
      while (state.phase === 'preview') {
        ...
        if (recorderRef.current) {
          await recorderRef.current.captureFrame(
            state.frameId * state.frameDuration,
            state.frameDuration
          )
        }
        state.frameId++
      ...
      while (state.phase === 'recording' && state.frameId < state.totalFrames) {
        ...
        if (
          exportMode === 'segmented' &&
          state.framesInCurrentSegment >= state.segmentDurationFrames
        ) {
  ...
  const startExport = useCallback(async () => {
    ...
    if (exportMode === 'stream') {
      ...
      if (!('showSaveFilePicker' in window)) {
        setError(
          'File System Access API not supported in this browser. Please use Chrome/Edge or switch to In-Memory mode.'
        )
        setStatus('error')
        exportStartedRef.current = false
        return
