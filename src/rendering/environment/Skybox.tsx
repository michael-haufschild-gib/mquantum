...
} from '@/rendering/shaders/skybox/compose'
...
const skyboxAssets = import.meta.glob('/src/assets/skyboxes/**/*.ktx2', {
  eager: true,
  import: 'default',
  query: '?url',
...
export const SkyboxMesh: React.FC<SkyboxMeshProps> = ({ texture }) => {
   ...
   */
  const computeColorAtT = (
    t: number,
    algorithm: ColorAlgorithm,
    coeffs: CosineCoefficients,
  ...
  const color1Vec = useMemo(() => {
    ...
    return computeColorAtT(0.0, 'cosine', skyboxCoeffs, skyboxDist, '#ffffff', 0.5, 0.15)
  }, [
    skyboxMode,
  ...
  const color2Vec = useMemo(() => {
    ...
    const skyboxCoeffs = proceduralSettings.cosineCoefficients
    const skyboxDist = proceduralSettings.distribution
    return computeColorAtT(1.0, 'cosine', skyboxCoeffs, skyboxDist, '#ffffff', 0.5, 0.15)
  }, [
    skyboxMode,
    proceduralSettings.syncWithObject,
    proceduralSettings.cosineCoefficients,
  ...
  useFrame((state, delta) => {
    ...
    if (isPlaying) {
      ...
      const speed =
        skyboxMode === 'classic' && skyboxAnimationMode !== 'none' ? skyboxAnimationSpeed : 1.0
    ...
    if (uniforms.uDistortion)
      uniforms.uDistortion.value = finalDistortion || proceduralSettings.turbulence
    ...
    if (skyboxChanged || appearanceChanged) {
      ...
      if (
        uniforms.uSunPosition?.value &&
        typeof (uniforms.uSunPosition.value as THREE.Vector3).set === 'function'
      ) {
        ;(uniforms.uSunPosition.value as THREE.Vector3).set(...proceduralSettings.sunPosition)
      }

      // Aurora settings
      if (uniforms.uAuroraCurtainHeight)
        uniforms.uAuroraCurtainHeight.value = proceduralSettings.aurora?.curtainHeight ?? 0.5
      if (uniforms.uAuroraWaveFrequency)
        uniforms.uAuroraWaveFrequency.value = proceduralSettings.aurora?.waveFrequency ?? 1.0

      // Horizon gradient settings
      if (uniforms.uHorizonGradientContrast)
        uniforms.uHorizonGradientContrast.value =
          proceduralSettings.horizonGradient?.gradientContrast ?? 0.5
      if (uniforms.uHorizonSpotlightFocus)
        uniforms.uHorizonSpotlightFocus.value =
          proceduralSettings.horizonGradient?.spotlightFocus ?? 0.5

      // Ocean settings
      if (uniforms.uOceanCausticIntensity)
        uniforms.uOceanCausticIntensity.value = proceduralSettings.ocean?.causticIntensity ?? 0.5
      if (uniforms.uOceanDepthGradient)
        uniforms.uOceanDepthGradient.value = proceduralSettings.ocean?.depthGradient ?? 0.5
      if (uniforms.uOceanBubbleDensity)
        uniforms.uOceanBubbleDensity.value = proceduralSettings.ocean?.bubbleDensity ?? 0.3
      if (uniforms.uOceanSurfaceShimmer)
        uniforms.uOceanSurfaceShimmer.value = proceduralSettings.ocean?.surfaceShimmer ?? 0.4
...
const SkyboxLoader: React.FC = () => {
  ...
  return (
    <>
      ...
      {shouldRenderSkybox && texture && <SkyboxMesh texture={texture} />}
    </>
  )
}
