...
const MandelbulbMesh = () => {
  ...
  const sliceAnimationEnabled = useExtendedObjectStore(
    (state) => state.mandelbulb.sliceAnimationEnabled
  )
      ...
      if (meshRef.current) {
        ...
        if (material.uniforms?.uProfileMode) {
          ...
          if (import.meta.env.DEV) {
            console.log(
              `Profile mode set to ${mode}: ${['Normal', 'Raymarch only', 'Raymarch+Normal', 'Raymarch+Normal+AO'][mode] || 'Unknown'}`
            )
          }
        }
      }
      ...
      if (meshRef.current) {
        ...
        if (material.uniforms.uEffectivePower) {
          ...
          if (mbConfig.alternatePowerEnabled) {
            effectivePower =
              effectivePower * (1 - mbConfig.alternatePowerBlend) +
        ...
        if (temporalUniforms) {
          ...
          if (material.uniforms.uPrevViewProjectionMatrix) {
            material.uniforms.uPrevViewProjectionMatrix.value.copy(
              temporalUniforms.uPrevViewProjectionMatrix
            )
          }
          if (material.uniforms.uPrevInverseViewProjectionMatrix) {
            material.uniforms.uPrevInverseViewProjectionMatrix.value.copy(
              temporalUniforms.uPrevInverseViewProjectionMatrix
            )
          }
          if (material.uniforms.uDepthBufferResolution) {
            material.uniforms.uDepthBufferResolution.value.copy(
              temporalUniforms.uDepthBufferResolution
            )
          }
        }
        ...
        if (appearanceChanged) {
          ...
          if (material.uniforms.uSssEnabled)
            material.uniforms.uSssEnabled.value = appearanceState.sssEnabled
          if (material.uniforms.uSssIntensity)
            material.uniforms.uSssIntensity.value = appearanceState.sssIntensity
          if (material.uniforms.uSssColor) {
            updateLinearColorUniform(
              ...
              appearanceState.sssColor || '#ff8844'
            )
          }
          if (material.uniforms.uSssThickness)
            material.uniforms.uSssThickness.value = appearanceState.sssThickness
          if (material.uniforms.uSssJitter)
            material.uniforms.uSssJitter.value = appearanceState.sssJitter
        ...
        if (material.uniforms.uShadowQuality) {
          ...
          const effectiveShadowQuality = getEffectiveShadowQuality(shadowQuality, qualityMultiplier)
