...
export const PolytopeScene = React.memo(function PolytopeScene({
  ...
  useEffect(() => {
    const unsubAnim = useAnimationStore.subscribe((s) => {
      animationStateRef.current = s
    })
  ...
  const {
    glsl: faceFragmentShader,
    modules: faceShaderModules,
  ...
  const faceVertexShader = useMemo(() => {
    return useScreenSpaceNormals ? buildFaceVertexShaderScreenSpace() : buildFaceVertexShader()
  ...
  useEffect(() => {
    ...
    if (facesVisible) {
      ...
      features.push(
        `Normals: ${useScreenSpaceNormals ? 'Screen-Space (dFdx/dFdy)' : 'Geometry-Based'}`
      )
    } else {
  ...
  useFrame(({ camera, scene }) => {
    ...
    const updateMeshMaterial = (
      ...
      if (appearanceChanged) {
        ...
        if (u.uRimColor)
          updateLinearColorUniform(cache.rimColor, u.uRimColor.value as Color, rimColor)
        ...
        if (u.uSssColor)
          updateLinearColorUniform(cache.sssColor, u.uSssColor.value as Color, sssColor)
