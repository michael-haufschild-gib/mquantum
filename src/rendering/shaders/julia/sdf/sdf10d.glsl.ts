...
float sdfJulia10D(vec3 pos, float pwr, float bail, int maxIt, out float trap) {
    // 10D initialization - z starts at sample point
    ...
    float c4 = 0.0, c5 = 0.0, c6 = 0.0, c7 = 0.0, c8 = 0.0, c9 = 0.0;
    ...
    for (int i = 0; i < MAX_ITER_HQ; i++) {
        ...
        float z5_sq = z5*z5, z6_sq = z6*z6, z7_sq = z7*z7, z8_sq = z8*z8, z9_sq = z9*z9;
        float z01_sq = z0_sq + z1_sq;
        float rSq = z01_sq + z2_sq + z3_sq + z4_sq + z5_sq + z6_sq + z7_sq + z8_sq + z9_sq;
        r = sqrt(rSq);
        ...
        float s0, c0_, s1, c1_, s2, c2_, s3, c3_, s4, c4_, s5, c5_, s6, c6_, s7, c7_, s8, c8_;
        ...
        if (usePower2) {
            ...
            float den89 = max(z8_sq + z9_sq, EPS*EPS);
            float invDen89 = 1.0 / den89;
            c8_ = (z8_sq - z9_sq) * invDen89;
            s8 = 2.0 * z8 * z9 * invDen89;
        } else {
            ...
            invTail = inversesqrt(max(tailSq, EPS*EPS));
            float t7 = acos(clamp(z7 * invTail, -1.0, 1.0));
            float t8 = atan(z9, z8);
...
float sdfJulia10D_simple(vec3 pos, float pwr, float bail, int maxIt) {
    float z0 = uOrigin[0] + pos.x*uBasisX[0] + pos.y*uBasisY[0] + pos.z*uBasisZ[0];
    float z1 = uOrigin[1] + pos.x*uBasisX[1] + pos.y*uBasisY[1] + pos.z*uBasisZ[1];
    float z2 = uOrigin[2] + pos.x*uBasisX[2] + pos.y*uBasisY[2] + pos.z*uBasisZ[2];
    float z3 = uOrigin[3] + pos.x*uBasisX[3] + pos.y*uBasisY[3] + pos.z*uBasisZ[3];
    ...
    for (int i = 0; i < MAX_ITER_HQ; i++) {
        ...
        float z0_sq = z0*z0, z1_sq = z1*z1, z2_sq = z2*z2, z3_sq = z3*z3, z4_sq = z4*z4;
        float z5_sq = z5*z5, z6_sq = z6*z6, z7_sq = z7*z7, z8_sq = z8*z8, z9_sq = z9*z9;
        float rSq = z0_sq + z1_sq + z2_sq + z3_sq + z4_sq + z5_sq + z6_sq + z7_sq + z8_sq + z9_sq;
        r = sqrt(rSq);
        if (r > bail) break;
