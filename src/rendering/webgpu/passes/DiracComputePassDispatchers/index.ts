    ...
    queueMicrotask(() =>
      staging
        ...
        .then(() => {
          ...
          const safeParticleNorm = finiteNonNegativeReadbackOrZero(particleNorm)
          const safeAntiNorm = finiteNonNegativeReadbackOrZero(antiNorm)

          // Asymmetric maxDensity smoothing
          currentMaxDensity = smoothPositiveReadbackPeak(currentMaxDensity, safeMaxDens)
          ...
          if (config.diagnosticsEnabled) {
            const norm0 = isFinitePositiveNorm(currentInitialNorm)
              ...
              : safeTotalNorm
            const normDrift =
              norm0 > 0 ? finiteReadbackRatioOrZero(safeTotalNorm - norm0, norm0) : 0
            const pFrac = finiteUnitReadbackRatioOrZero(safeParticleNorm, safeTotalNorm)
            const aFrac = finiteUnitReadbackRatioOrZero(safeAntiNorm, safeTotalNorm)
