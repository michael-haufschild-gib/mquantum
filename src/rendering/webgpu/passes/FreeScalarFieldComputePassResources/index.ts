...
import { projectSimEta } from '../fsfCosmologyStepping'
...
/**
 * Create a COPY_SRC staging buffer for the 24-byte cosmology/preheating
 * coefficient slot. Used by initialization kickstart so coefficient updates
 * are ordered in the command buffer, matching the runtime leapfrog path.
 */
export function createCosmologyCoefsStagingBuffer(
  device: GPUDevice,
  label: string,
  values: readonly [number, number, number, number, number, number]
): GPUBuffer {
  const staging = device.createBuffer({
    label: `free-scalar-${label}-cosmo-coefs-staging`,
    size: FSF_COSMO_COEFS_BYTE_SIZE,
    usage: GPUBufferUsage.COPY_SRC,
    mappedAtCreation: true,
  })
...
export function initializeFsfField(
  ...
  if (!injectedFromSave && ic.pl && ic.bg && ic.uniformBuffer) {
    ...
    let coefsStaging: GPUBuffer | null = null
    if (cosmoOn || preheatingOn) {
      ...
      const massSquaredScale = preheatingOn
        ...
        : 1
      coefsStaging = createCosmologyCoefsStagingBuffer(device, 'kickstart', [
        aKinetic,
        aPotential,
        aFull,
        massSquaredScale,
    ...
    if (coefsStaging) ic.pendingStagingBuffers.push(coefsStaging)
