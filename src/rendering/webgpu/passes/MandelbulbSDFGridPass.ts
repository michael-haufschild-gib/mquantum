/**
 * Mandelbulb SDF Grid Compute Pass
 *
 * Pre-computes a 3D SDF texture from the Mandelbulb fractal using a compute shader.
 ...
 *
 * @module rendering/webgpu/passes/MandelbulbSDFGridPass
 */

import { WebGPUBaseComputePass } from '../core/WebGPUBasePass'
import type { WebGPURenderContext, WebGPUSetupContext } from '../core/types'
import { composeMandelbulbSDFGridShader } from '../shaders/mandelbulb/compute/compose'
...
// MandelbulbUniforms size: matches renderer
// See uniforms.wgsl.ts: MandelbulbUniforms struct
// Total: ~80 bytes padded to 256 for safety
const MANDELBULB_UNIFORMS_SIZE = 256

// BasisVectors size: 192 bytes (4 × 3 × vec4f)
const BASIS_VECTORS_SIZE = 192

/**
 * Configuration for the Mandelbulb SDF grid compute pass.
 */
export interface MandelbulbSDFGridConfig {
...
/**
 * Compute pass that pre-computes a 3D SDF texture from Mandelbulb fractal.
 *
 ...
 */
export class MandelbulbSDFGridPass extends WebGPUBaseComputePass {
  // Configuration
  private passConfig: MandelbulbSDFGridConfig
  ...
  private mandelbulbBuffer: GPUBuffer | null = null
  ...
  constructor(config: MandelbulbSDFGridConfig) {
    super({
      id: 'mandelbulb-sdf-grid-compute',
  ...
  protected async createPipeline(ctx: WebGPUSetupContext): Promise<void> {
    ...
    const { wgsl } = composeMandelbulbSDFGridShader({
      dimension: this.passConfig.dimension,
    })

    // Create shader module
    const shaderModule = this.createShaderModule(device, wgsl, 'mandelbulb-sdf-grid-compute')
    ...
    this.sdfTexture = device.createTexture({
      label: 'mandelbulb-sdf-grid-texture',
      size: {
        width: this.gridSize,
    ...
    this.mandelbulbBuffer = this.createUniformBuffer(
      device,
      MANDELBULB_UNIFORMS_SIZE,
      'sdf-grid-mandelbulb'
    )
    ...
    this.computeBindGroup = device.createBindGroup({
      label: 'mandelbulb-sdf-grid-compute-bg',
      layout: this.computeBindGroupLayout,
      entries: [
        { binding: 0, resource: { buffer: this.mandelbulbBuffer } },
  ...
  /**
   * Update Mandelbulb uniforms from render context.
   * This copies the uniform data from the renderer.
   */
  updateMandelbulbUniforms(device: GPUDevice, data: ArrayBuffer): void {
    if (this.mandelbulbBuffer) {
      device.queue.writeBuffer(this.mandelbulbBuffer, 0, data)
      ...
      // changes (power, iterations, bailout, dimension) to decide when to recompute.
    }
  }
   ...
   */
  needsUpdate(power: number, iterations: number, bailout: number): boolean {
  ...
  execute(ctx: WebGPURenderContext): void {
    if (!this.computePipeline || !this.computeBindGroup) {
      console.warn('MandelbulbSDFGridPass: Pipeline not initialized')
      return
    }

    // Get current parameters from stores
    const mandelbulbStore = ctx.frame?.stores?.['mandelbulb'] as {
      power?: number
      iterations?: number
      bailout?: number
    } | undefined

    const power = mandelbulbStore?.power ?? 8
    const iterations = mandelbulbStore?.iterations ?? 50
    const bailout = mandelbulbStore?.bailout ?? 2.0

    // Early exit if no update needed
    if (!this.needsUpdate(power, iterations, bailout)) {
      return
    }

    // Create compute pass using context method
    const computePass = ctx.beginComputePass({
      label: 'mandelbulb-sdf-grid-compute-pass',
    })
  ...
  dispose(): void {
    ...
    this.mandelbulbBuffer?.destroy()
    this.mandelbulbBuffer = null
