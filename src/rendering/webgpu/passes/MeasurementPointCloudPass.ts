 ...
 * Reads measurement data from the measurement store and uploads to a GPU
 * storage buffer each frame. Uses additive blending for a glow effect.
 *
...
export class MeasurementPointCloudPass extends WebGPUBasePass {
  ...
  /**
   * Upload measurement positions and render.
   *
   * @param device - GPU device for buffer writes
   * @param positions - Array of [x, y, z] world positions
   * @param viewProjection - Combined VP matrix (Float32Array[16])
   * @param pointSize - Billboard size in clip space
   * @param opacity - Overall opacity
   */
  updatePositions(
    device: GPUDevice,
    positions: number[][],
    ...
    // Pack positions with age (0=newest, 1=oldest)
    ...
    device.queue.writeBuffer(this.positionBuffer!, 0, this.uploadData, 0, this.pointCount * 4)

    // Pack uniforms
    this.uniformData.set(viewProjection, 0)
    this.uniformData[16] = pointSize
  ...
  execute(ctx: WebGPURenderContext): void {
    if (!this.renderPipeline || !this.bindGroup || this.pointCount === 0) return
