        ...
        staging
          ...
          .then(() => {
            ...
            if (data.length >= DIAG_RESULT_COUNT) {
              ...
              const safeNormUp = finiteNonNegativeReadbackOrZero(normUp)
              const safeNormDown = finiteNonNegativeReadbackOrZero(normDown)
              const safeSigmaX = finiteReadbackOrZero(sigmaX)
              const safeSigmaY = finiteReadbackOrZero(sigmaY)
              const safeSigmaZ = finiteReadbackOrZero(sigmaZ)
              const safeMaxDensity = finiteNonNegativeReadbackOrZero(rawMaxDensity)
              ...
              this.maxDensity = smoothPositiveReadbackPeak(this.maxDensity, safeMaxDensity)

              const spinUpFraction = finiteUnitReadbackRatioOrZero(safeNormUp, safeTotalNorm)
              const spinDownFraction = finiteUnitReadbackRatioOrZero(safeNormDown, safeTotalNorm)
              const spinExpectationZ = Math.min(
                1,
                Math.max(-1, finiteReadbackRatioOrZero(safeSigmaZ, safeTotalNorm))
              )
              const coherenceMagnitude = finiteReadbackRatioOrZero(
                Math.hypot(safeSigmaX, safeSigmaY),
                safeTotalNorm
              )
              const safeHbar =
                Number.isFinite(this.cachedHbar) && Math.abs(this.cachedHbar) > 1e-9
                  ? this.cachedHbar
                  : 1.0
              const safeFieldStrength = Number.isFinite(this.cachedFieldStrength)
                ? this.cachedFieldStrength
                : 0
              const larmorFrequency = (2 * safeFieldStrength) / safeHbar
              ...
              const normDrift =
                isFinitePositiveNorm(this.initialNorm) && totalNormIsSafe
                  ? finiteReadbackRatioOrZero(totalNorm - this.initialNorm, this.initialNorm)
                  : 0
