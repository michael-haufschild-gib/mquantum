...
/** Finite-only real/imaginary wavefunction copy plus accumulated norm. */
export interface FiniteWavefunctionSnapshot {
  /** Real components copied from interleaved GPU readback data. */
  re: Float32Array
  /** Imaginary components copied from interleaved GPU readback data. */
  im: Float32Array
  /** Sum of squared component magnitudes over all copied sites. */
  norm: number
}

/** Copy an interleaved vec2f wavefunction only when every component is finite. */
export function copyFiniteWavefunctionSnapshot(
  interleaved: Float32Array,
  totalSites: number
): FiniteWavefunctionSnapshot | null {
  if (!Number.isSafeInteger(totalSites) || totalSites <= 0) return null
  if (interleaved.length < 2 * totalSites) return null

  const re = new Float32Array(totalSites)
  const im = new Float32Array(totalSites)
  let norm = 0
  for (let i = 0; i < totalSites; i++) {
    const r = interleaved[2 * i]!
    const m = interleaved[2 * i + 1]!
  ...
  device.queue
    ...
    .then(async () => {
      ...
      if (state.psi0Re === null || state.psi0Im === null) {
        ...
        const snapshot = copyFiniteWavefunctionSnapshot(interleaved, capturedTotalSites)
        if (!snapshot) {
          finish()
          return
        }
        state.psi0Re = snapshot.re
        state.psi0Im = snapshot.im
        state.baseSimTime = capturedSimTime

        // C(0) = ⟨ψ₀|ψ₀⟩ = Σ |ψ₀|². Record it as the t=0 anchor.
        pushAutocorrelationSample(state.buffer, snapshot.norm, 0, 0)
