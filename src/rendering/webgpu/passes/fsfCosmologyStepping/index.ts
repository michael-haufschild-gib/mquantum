...
export function computeFsfCflSubsteps(
  ...
  if (!Number.isFinite(config.dt) || config.dt <= 0) return 1
  const activeDims =
    Number.isSafeInteger(config.latticeDim) && config.latticeDim > 0
      ? Math.min(config.latticeDim, config.gridSize.length, config.spacing.length)
      : 0
  const scale =
    Number.isFinite(massSquaredScaleMax) && massSquaredScaleMax > 1 ? massSquaredScaleMax : 1
  const drift = Number.isFinite(aKinetic) && aKinetic > 0 ? aKinetic : 1
  const mass = Number.isFinite(config.mass) && config.mass > 0 ? config.mass : 0
  const full = Number.isFinite(aFull) && aFull > 0 ? aFull : 0
  const basePotential = Number.isFinite(aPotential) && aPotential >= 0 ? aPotential : 1
  const massStiffness = mass * mass * full * scale

  let stiffnessMax = massStiffness
  for (let d = 0; d < activeDims; d++) {
    const spacing = config.spacing[d]!
    const axisSize = config.gridSize[d]!
    if (!Number.isFinite(spacing) || spacing <= 0 || !Number.isInteger(axisSize) || axisSize <= 1) {
      continue
    }
    let axisPotential = basePotential
  ...
  const key = `${cosmo.preset}|d=${activeDims}|m=${mass}|dt=${config.dt}`
 ...
 *
 * The compute pass stages this bundle into the GPU uniform buffer via
 * encoder-ordered copies every drift→kick pair whenever either cosmology or
 * preheating is active, so the pi-update shader reads fresh values for the
 * time-dependent FLRW coefficients and the parametric resonance drive.
 */
