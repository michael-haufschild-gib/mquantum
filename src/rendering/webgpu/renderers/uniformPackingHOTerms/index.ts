...
export function packPrecomputedHOTerms(
  ...
  const rawTime = finiteOrZero(animationTime) * finiteOrZero(timeScale)
  const t = finiteOrZero(rawTime)
  const termCount = Math.min(Math.max(intView[I.termCount] ?? 0, 0), MAX_TERMS)

  for (let k = 0; k < termCount; k++) {
    const energy = finiteOrZero(floatView[I.energy + k] ?? 0)
    const coeffRe = finiteOrZero(floatView[I.coeff + k * 4] ?? 0)
    const coeffIm = finiteOrZero(floatView[I.coeff + k * 4 + 1] ?? 0)
    const phase = finiteOrZero(-energy * t)
    const cosP = Math.cos(phase)
    const sinP = Math.sin(phase)
    // term_k = c_k * (cos + i sin)
    //        = (Re*cos - Im*sin) + i (Re*sin + Im*cos)
    const slot = I.precomputedTerm + k * 4
    floatView[slot] = finiteOrZero(coeffRe * cosP - coeffIm * sinP)
    floatView[slot + 1] = finiteOrZero(coeffRe * sinP + coeffIm * cosP)
    floatView[slot + 2] = 0
    floatView[slot + 3] = 0
  }
