/**
 * WGSL Julia SDF - 11D Hyperspherical Power Map
 *
 * @module rendering/webgpu/shaders/julia/sdf11d.wgsl
 */
...
fn sdfJulia11D(pos: vec3f, pwr: f32, bail: f32, maxIt: i32) -> vec2f {
  ...
  var z9 = getBasisComponent(basis.origin, 9) + pos.x * getBasisComponent(basis.basisX, 9) + pos.y * getBasisComponent(basis.basisY, 9) + pos.z * getBasisComponent(basis.basisZ, 9);
  var z10 = getBasisComponent(basis.origin, 10) + pos.x * getBasisComponent(basis.basisX, 10) + pos.y * getBasisComponent(basis.basisY, 10) + pos.z * getBasisComponent(basis.basisZ, 10);

  let c0 = julia.juliaConstant.x; let c1 = julia.juliaConstant.y;
  let c2 = julia.juliaConstant.z; let c3 = julia.juliaConstant.w;
  let c4 = 0.0; let c5 = 0.0; let c6 = 0.0; let c7 = 0.0; let c8 = 0.0; let c9 = 0.0; let c10 = 0.0;
  ...
  for (var i = 0; i < MAX_ITER_HQ; i++) {
    ...
    let z0_sq = z0*z0; let z1_sq = z1*z1; let z2_sq = z2*z2; let z3_sq = z3*z3; let z4_sq = z4*z4;
    let z5_sq = z5*z5; let z6_sq = z6*z6; let z7_sq = z7*z7; let z8_sq = z8*z8; let z9_sq = z9*z9; let z10_sq = z10*z10;
    let z01_sq = z0_sq + z1_sq;
    let rSq = z01_sq + z2_sq + z3_sq + z4_sq + z5_sq + z6_sq + z7_sq + z8_sq + z9_sq + z10_sq;
    r = sqrt(rSq);
    ...
    var s8: f32; var c8_: f32; var s9: f32; var c9_: f32;
    ...
    if (usePower2) {
      ...
      invTail = inverseSqrt(max(tailSq, EPS*EPS));
      arg = clamp(z7 * invTail, -1.0, 1.0);
      c7_ = 2.0*arg*arg - 1.0; s7 = 2.0*arg*sqrt(max(1.0 - arg*arg, 0.0));
      tailSq = max(tailSq - z7_sq, 0.0);

      invTail = inverseSqrt(max(tailSq, EPS*EPS));
      arg = clamp(z8 * invTail, -1.0, 1.0);
      c8_ = 2.0*arg*arg - 1.0; s8 = 2.0*arg*sqrt(max(1.0 - arg*arg, 0.0));

      let den910 = max(z9_sq + z10_sq, EPS*EPS);
      c9_ = (z9_sq - z10_sq) / den910;
      s9 = 2.0 * z9 * z10 / den910;
    } else {
      var tailSq = rSq;
      var invTail = inverseSqrt(max(tailSq, EPS*EPS));
      ...
      invTail = inverseSqrt(max(tailSq, EPS*EPS));
      let t6 = acos(clamp(z6 * invTail, -1.0, 1.0)); tailSq = max(tailSq - z6_sq, 0.0);
      invTail = inverseSqrt(max(tailSq, EPS*EPS));
      let t7 = acos(clamp(z7 * invTail, -1.0, 1.0)); tailSq = max(tailSq - z7_sq, 0.0);
      invTail = inverseSqrt(max(tailSq, EPS*EPS));
      let t8 = acos(clamp(z8 * invTail, -1.0, 1.0));
      let t9 = atan2(z10, z9);
...
fn sdfJulia11D_simple(pos: vec3f, pwr: f32, bail: f32, maxIt: i32) -> f32 {
  var z0 = getBasisComponent(basis.origin, 0) + pos.x * getBasisComponent(basis.basisX, 0) + pos.y * getBasisComponent(basis.basisY, 0) + pos.z * getBasisComponent(basis.basisZ, 0);
  var z1 = getBasisComponent(basis.origin, 1) + pos.x * getBasisComponent(basis.basisX, 1) + pos.y * getBasisComponent(basis.basisY, 1) + pos.z * getBasisComponent(basis.basisZ, 1);
  var z2 = getBasisComponent(basis.origin, 2) + pos.x * getBasisComponent(basis.basisX, 2) + pos.y * getBasisComponent(basis.basisY, 2) + pos.z * getBasisComponent(basis.basisZ, 2);
  var z3 = getBasisComponent(basis.origin, 3) + pos.x * getBasisComponent(basis.basisX, 3) + pos.y * getBasisComponent(basis.basisY, 3) + pos.z * getBasisComponent(basis.basisZ, 3);
  ...
  for (var i = 0; i < MAX_ITER_HQ; i++) {
    ...
    let z0_sq = z0*z0; let z1_sq = z1*z1; let z2_sq = z2*z2; let z3_sq = z3*z3; let z4_sq = z4*z4;
    let z5_sq = z5*z5; let z6_sq = z6*z6; let z7_sq = z7*z7; let z8_sq = z8*z8; let z9_sq = z9*z9; let z10_sq = z10*z10;
    let rSq = z0_sq + z1_sq + z2_sq + z3_sq + z4_sq + z5_sq + z6_sq + z7_sq + z8_sq + z9_sq + z10_sq;
    r = sqrt(rSq);
