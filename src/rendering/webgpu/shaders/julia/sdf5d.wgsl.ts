/**
 * WGSL Julia SDF - 5D Hyperspherical Power Map
 *
 * Port of GLSL julia/sdf/sdf5d.glsl to WGSL.
 * z = z^n + c where z starts at sample point, c is Julia constant
 *
 * @module rendering/webgpu/shaders/julia/sdf5d.wgsl
 */
...
fn sdfJulia5D(
  ...
  for (var i = 0; i < MAX_ITER_HQ; i++) {
    ...
    let rSq = zxzy_sq + zz_sq + z3_sq + z4_sq;
    r = sqrt(rSq);
    ...
    if (usePower2) {
      ...
      // atan angle-doubling
      let den34 = max(z3_sq + z4_sq, EPS * EPS);
      let invDen34 = 1.0 / den34;
      c3_ = (z3_sq - z4_sq) * invDen34;
      s3 = 2.0 * z3 * z4 * invDen34;
    } else {
      // General power path
      var tailSq = rSq;
      var invTail = inverseSqrt(max(tailSq, EPS * EPS));
...
fn sdfJulia5D_simple(pos: vec3f, pwr: f32, bail: f32, maxIt: i32) -> f32 {
  var zx = getBasisComponent(basis.origin, 0) +
           pos.x * getBasisComponent(basis.basisX, 0) +
           pos.y * getBasisComponent(basis.basisY, 0) +
           pos.z * getBasisComponent(basis.basisZ, 0);
  ...
  for (var i = 0; i < MAX_ITER_HQ; i++) {
    ...
    let z3_sq = z3 * z3;
    let z4_sq = z4 * z4;
    let rSq = zx_sq + zy_sq + zz_sq + z3_sq + z4_sq;
    r = sqrt(rSq);
