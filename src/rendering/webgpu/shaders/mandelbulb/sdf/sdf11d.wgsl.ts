...
fn optimizedPow11D(r: f32, p: f32) -> vec2f {
  let logR = log(max(r, EPS_11D));
  let rp = exp(logR * p);
...
fn mandelbulbSDF11D(
  ...
  for (var i = 0; i < MAX_ITER_11D; i++) {
    ...
    zSq[8] = z[8] * z[8]; zSq[9] = z[9] * z[9]; zSq[10] = z[10] * z[10];

    r = sqrt(z01_sq + zSq[2] + zSq[3] + zSq[4] + zSq[5] + zSq[6] + zSq[7] + zSq[8] + zSq[9] + zSq[10]);
    ...
    let powers = optimizedPow11D(r, pwr);
    let rp = powers.x;
    let rpMinus1 = powers.y;
    dr = rpMinus1 * pwr * dr + 1.0;

    // 11D: 10 angles using inverseSqrt optimization
    var t: array<f32, 10>;
    var tailSq = r * r;

    for (var k = 0; k < 9; k++) {
      let invTail = inverseSqrt(max(tailSq, EPS_11D * EPS_11D));
    ...
    let s9 = sin(t[9] * pwr); let c9 = cos(t[9] * pwr);
