...
export interface SchroedingerWGSLShaderConfig extends WGSLShaderConfig {
  ...
  /**
   * Arithmetic Horizon mode (Riemann ζ spectral synthesis). Selects the
   * dedicated volumetric main block and excludes the quantum-math and volume
   * blocks entirely — the mode's shader is self-contained (uniforms + basis +
   * HSL + sphere intersection + a group-2 radial LUT storage buffer). The
   * top-level `quantumMode` is narrowed to `'harmonicOscillator'` like AdS /
  ...
  /**
   * Bifurcation Horizon mode (Kruskal eternal black hole on the Riemann
   * critical strip). Selects the dedicated volumetric main block and excludes
   * the quantum-math and volume blocks entirely — the mode's shader is
   * self-contained (uniforms + basis + HSL + sphere intersection + a group-2
   * 2D (t, u) LUT storage buffer). The top-level `quantumMode` is narrowed to
   * `'harmonicOscillator'` like the other horizon modes. Leave `false` for
   * every other mode.
   */
