/**
 * Wigner Spatial Precompute Shader Composer
 *
 * Assembles the compute shader for Phase 1 (spatial precompute) of the
 * two-phase Wigner cache pipeline. Includes all quantum math modules
 * needed for wignerDiagonal/wignerCross evaluation.
 *
...
// Shared grid params struct (reused from original cache shader)
import { wignerGridParamsBlock } from './wignerCache.wgsl'

// Spatial-specific blocks
import {
  wignerSpatialParamsBlock,
  generateWignerSpatialBindingsBlock,
...
/**
 * Configuration for the Wigner spatial precompute shader
 */
export interface WignerSpatialComputeConfig {
...
/**
 * Compose the Wigner spatial precompute compute shader.
 *
 * Identical quantum math module set as the original composeWignerCache,
 * but writes diagonal + cross-term textures instead of a single cache.
 *
 * @param config - Shader configuration
 * @returns Composed WGSL code and metadata
 */
export function composeWignerSpatialComputeShader(config: WignerSpatialComputeConfig): {
  wgsl: string
  modules: string[]
  features: string[]
} {
  ...
  features.push('Wigner Spatial Precompute')
  ...
  const blocks = [
    ...
    // Wigner grid params struct (reused)
    { name: 'Wigner Grid Params', content: wignerGridParamsBlock },

    // Spatial params struct
    { name: 'Spatial Params', content: wignerSpatialParamsBlock },

    // Compute shader bindings
    { name: 'Spatial Bindings', content: generateWignerSpatialBindingsBlock() },
    ...
    { name: 'Spatial Compute Main', content: wignerSpatialComputeBlock },
  ]
