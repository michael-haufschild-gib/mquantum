...
fn curvedWarpedRicci(r: f32, rPrime: f32, rDoublePrime: f32, dim: u32) -> f32 {
  let d1 = f32(dim - 1u);
  let d2 = f32(dim) - 2.0;
...
fn ricciScalarWGSL(coords: array<f32, 12>, dim: u32, time: f32) -> f32 {
  ...
  if (kind == 1u) {
    ...
    return curvedWarpedRicci(r, l / r, (b0 * b0) / (r * r * r), dim);
  }
  if (kind == 7u) {
    let b0 = max(params.doubleThroatRad, 0.1);
    let s = max(params.doubleThroatSep, 0.2);
    ...
    return curvedWarpedRicci(r, rPrime, rDoublePrime, dim);
  }
  return 0.0;
}
