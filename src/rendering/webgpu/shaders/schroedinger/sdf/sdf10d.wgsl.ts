/**
 * WGSL 10D Hyperbulb SDF for Schrödinger isosurface rendering
 *
 * Port of GLSL schroedinger/sdf/sdf10d.glsl to WGSL.
 * Uses BasisVectors pattern matching Mandelbulb for N-D transformation.
 *
 * @module rendering/webgpu/shaders/schroedinger/sdf/sdf10d.wgsl
 */

export const sdf10dBlock = /* wgsl */ `
// ============================================
// 10D Hyperbulb SDF - FULLY UNROLLED
// ============================================
...
/**
 * 10D Mandelbulb-style SDF with orbital trap.
 */
fn sdf10D(
  pos: vec3f,
  pwr: f32,
  ...
  for (var i = 0; i < MAX_ITER_10D; i++) {
    ...
    dr = pow(max(r, EPS_10D), pwr - 1.0) * pwr * dr + 1.0;

    // 10D: 9 angles (t0..t7 from acos, t8 from atan2)
    var tail = r;
    let t0 = acos(clamp(z0 / max(tail, EPS_10D), -1.0, 1.0)); tail = sqrt(max(tail*tail - z0*z0, EPS_10D));
    let t1 = acos(clamp(z1 / max(tail, EPS_10D), -1.0, 1.0)); tail = sqrt(max(tail*tail - z1*z1, EPS_10D));
    let t2 = acos(clamp(z2 / max(tail, EPS_10D), -1.0, 1.0)); tail = sqrt(max(tail*tail - z2*z2, EPS_10D));
    let t3 = acos(clamp(z3 / max(tail, EPS_10D), -1.0, 1.0)); tail = sqrt(max(tail*tail - z3*z3, EPS_10D));
    let t4 = acos(clamp(z4 / max(tail, EPS_10D), -1.0, 1.0)); tail = sqrt(max(tail*tail - z4*z4, EPS_10D));
    let t5 = acos(clamp(z5 / max(tail, EPS_10D), -1.0, 1.0)); tail = sqrt(max(tail*tail - z5*z5, EPS_10D));
    let t6 = acos(clamp(z6 / max(tail, EPS_10D), -1.0, 1.0)); tail = sqrt(max(tail*tail - z6*z6, EPS_10D));
    let t7 = acos(clamp(z7 / max(tail, EPS_10D), -1.0, 1.0));
    let t8 = atan2(z9, z8);

    let rp = pow(max(r, EPS_10D), pwr);
...
/**
 * 10D SDF - simple version without trap.
 */
fn sdf10D_simple(
  pos: vec3f,
  pwr: f32,
