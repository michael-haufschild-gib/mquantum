/**
 * WGSL 11D Hyperbulb SDF for Schrödinger isosurface rendering
 *
 * Port of GLSL schroedinger/sdf/sdf11d.glsl to WGSL.
 * Uses BasisVectors pattern matching Mandelbulb for N-D transformation.
 *
 * @module rendering/webgpu/shaders/schroedinger/sdf/sdf11d.wgsl
 */

export const sdf11dBlock = /* wgsl */ `
// ============================================
// 11D Hyperbulb SDF - FULLY UNROLLED
// ============================================

const MAX_ITER_11D: i32 = 256;
const EPS_11D: f32 = 1e-6;

/**
 * 11D Mandelbulb-style SDF with orbital trap.
 */
fn sdf11D(
  pos: vec3f,
  pwr: f32,
  ...
  let c9 = getBasisComponent(basis.origin, 9) + pos.x * getBasisComponent(basis.basisX, 9) + pos.y * getBasisComponent(basis.basisY, 9) + pos.z * getBasisComponent(basis.basisZ, 9);
  let c10 = getBasisComponent(basis.origin, 10) + pos.x * getBasisComponent(basis.basisX, 10) + pos.y * getBasisComponent(basis.basisY, 10) + pos.z * getBasisComponent(basis.basisZ, 10);
  ...
  for (var i = 0; i < MAX_ITER_11D; i++) {
    ...
    dr = pow(max(r, EPS_11D), pwr - 1.0) * pwr * dr + 1.0;

    // 11D: 10 angles (t0..t8 from acos, t9 from atan2)
    var tail = r;
    let t0 = acos(clamp(z0 / max(tail, EPS_11D), -1.0, 1.0)); tail = sqrt(max(tail*tail - z0*z0, EPS_11D));
    let t1 = acos(clamp(z1 / max(tail, EPS_11D), -1.0, 1.0)); tail = sqrt(max(tail*tail - z1*z1, EPS_11D));
    let t2 = acos(clamp(z2 / max(tail, EPS_11D), -1.0, 1.0)); tail = sqrt(max(tail*tail - z2*z2, EPS_11D));
    let t3 = acos(clamp(z3 / max(tail, EPS_11D), -1.0, 1.0)); tail = sqrt(max(tail*tail - z3*z3, EPS_11D));
    let t4 = acos(clamp(z4 / max(tail, EPS_11D), -1.0, 1.0)); tail = sqrt(max(tail*tail - z4*z4, EPS_11D));
    let t5 = acos(clamp(z5 / max(tail, EPS_11D), -1.0, 1.0)); tail = sqrt(max(tail*tail - z5*z5, EPS_11D));
    let t6 = acos(clamp(z6 / max(tail, EPS_11D), -1.0, 1.0)); tail = sqrt(max(tail*tail - z6*z6, EPS_11D));
    let t7 = acos(clamp(z7 / max(tail, EPS_11D), -1.0, 1.0)); tail = sqrt(max(tail*tail - z7*z7, EPS_11D));
    let t8 = acos(clamp(z8 / max(tail, EPS_11D), -1.0, 1.0));
    let t9 = atan2(z10, z9);

    let rp = pow(max(r, EPS_11D), pwr);
    ...
    z7 = sp * cos(t7 * pwr) + c7; sp *= sin(t7 * pwr);
    z8 = sp * cos(t8 * pwr) + c8; sp *= sin(t8 * pwr);
    z9 = sp * cos(t9 * pwr) + c9;
    z10 = sp * sin(t9 * pwr) + c10;
...
/**
 * 11D SDF - simple version without trap.
 */
fn sdf11D_simple(
  pos: vec3f,
  pwr: f32,
