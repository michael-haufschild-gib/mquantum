/**
 * WGSL 8D Hyperbulb SDF for Schrödinger isosurface rendering
 *
 * Port of GLSL schroedinger/sdf/sdf8d.glsl to WGSL.
 * Uses BasisVectors pattern matching Mandelbulb for N-D transformation.
 *
 * @module rendering/webgpu/shaders/schroedinger/sdf/sdf8d.wgsl
 */

export const sdf8dBlock = /* wgsl */ `
// ============================================
// 8D Hyperbulb SDF - FULLY UNROLLED
// ============================================
...
/**
 * 8D Mandelbulb-style SDF with orbital trap.
 */
fn sdf8D(
  pos: vec3f,
  pwr: f32,
  ...
  for (var i = 0; i < MAX_ITER_8D; i++) {
    ...
    r = sqrt(zx * zx + zy * zy + zz * zz + z3 * z3 + z4 * z4 + z5 * z5 + z6 * z6 + z7 * z7);
    if (r > bail) { escIt = i; break; }

    minP = min(minP, abs(zy));
    minA = min(minA, sqrt(zx * zx + zy * zy));
    minS = min(minS, abs(r - 0.8));
    dr = pow(max(r, EPS_8D), pwr - 1.0) * pwr * dr + 1.0;

    // 8D: 7 angles, z-axis primary
    let t0 = acos(clamp(zz / max(r, EPS_8D), -1.0, 1.0));
    let r1 = sqrt(zx * zx + zy * zy + z3 * z3 + z4 * z4 + z5 * z5 + z6 * z6 + z7 * z7);
    let t1 = select(0.0, acos(clamp(zx / max(r1, EPS_8D), -1.0, 1.0)), r1 > EPS_8D);
    let r2 = sqrt(zy * zy + z3 * z3 + z4 * z4 + z5 * z5 + z6 * z6 + z7 * z7);
    let t2 = select(0.0, acos(clamp(zy / max(r2, EPS_8D), -1.0, 1.0)), r2 > EPS_8D);
    let r3 = sqrt(z3 * z3 + z4 * z4 + z5 * z5 + z6 * z6 + z7 * z7);
    let t3 = select(0.0, acos(clamp(z3 / max(r3, EPS_8D), -1.0, 1.0)), r3 > EPS_8D);
    let r4 = sqrt(z4 * z4 + z5 * z5 + z6 * z6 + z7 * z7);
    let t4 = select(0.0, acos(clamp(z4 / max(r4, EPS_8D), -1.0, 1.0)), r4 > EPS_8D);
    let r5 = sqrt(z5 * z5 + z6 * z6 + z7 * z7);
    let t5 = select(0.0, acos(clamp(z5 / max(r5, EPS_8D), -1.0, 1.0)), r5 > EPS_8D);
    let t6 = atan2(z7, z6);

    let rp = pow(max(r, EPS_8D), pwr);
    ...
    let sp = rp * s0 * s1 * s2 * s3 * s4 * s5;
    zz = rp * c0 + cz;
    zx = rp * s0 * c1 + cx;
    zy = rp * s0 * s1 * c2 + cy;
    z3 = rp * s0 * s1 * s2 * c3_ + c3;
    z4 = rp * s0 * s1 * s2 * s3 * c4_ + c4;
    z5 = rp * s0 * s1 * s2 * s3 * s4 * c5_ + c5;
    z6 = sp * c6_ + c6;
    z7 = sp * s6 + c7;
    escIt = i;
  }
...
/**
 * 8D SDF - simple version without trap.
 */
fn sdf8D_simple(
  pos: vec3f,
  pwr: f32,
