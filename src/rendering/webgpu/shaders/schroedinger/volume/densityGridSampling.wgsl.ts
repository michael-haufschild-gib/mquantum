...
fn worldToDensityGridUVW(pos: vec3f, uniforms: SchroedingerUniforms) -> vec3f {
  ...
  // Free-scalar density textures are written in lattice-local axes, so sampling must
  // apply the current basis/origin transform. Hydrogen density textures already bake
  // basis/origin during compute write (mapPosToND), so remapping again would distort
  // lobes by double-transforming coordinates.
  if (IS_FREE_SCALAR) {
    gridPos = vec3f(
