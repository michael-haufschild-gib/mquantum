...
fn evaluateIsolines2D(pos: vec3f, rho: f32, s: f32, uniforms: SchroedingerUniforms) -> vec4f {
  ...
  // Contour color: bright white/gray lines for visibility; dim for lower density.
  // 0.3 + 0.7 * [0,1] is already bounded to [0.3, 1.0] so the outer clamp was redundant.
  let levelBrightness = 0.3 + 0.7 * clamp((s + 6.0) / 6.0, 0.0, 1.0);
