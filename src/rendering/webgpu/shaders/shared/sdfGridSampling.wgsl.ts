...
/**
 * Hybrid dispatch that falls back to direct evaluation outside grid bounds.
 * This provides better quality at the edges but requires the original SDF
 * function to be available.
 *
 * @param dimension The fractal dimension for the fallback function
 */
export function generateHybridSDFDispatch(dimension: number): string {
  const sdfFnName =
    dimension >= 3 && dimension <= 11
      ? `mandelbulbSDF${dimension}D`
      : 'mandelbulbSDF3D'
  const sdfSimpleFnName = `${sdfFnName}_simple`

  return /* wgsl */ `
// ============================================
// Hybrid SDF Dispatch (Grid + Fallback)
// ============================================
// Uses grid sampling inside bounds, direct evaluation outside

fn GetDist(p: vec3f) -> f32 {
  // Try grid sampling first
  if (isInsideSDFGrid(p)) {
    return GetDistFromGrid(p);
  }

  // Fallback to direct evaluation outside grid
  return ${sdfSimpleFnName}(p, basis, mandelbulb);
}
...
fn GetDistWithOrbital(p: vec3f) -> vec2f {
  ...
  // Fallback to direct evaluation outside grid
  return ${sdfFnName}(p, basis, mandelbulb);
}
`
}
