...
describe('fft throws on invalid input', () => {
  ...
  it('rejects oversized powers of two before bit-reversal uses signed shifts', () => {
    const data = new Float64Array(8)
    expect(() => fft(data, 2 ** 31)).toThrow(/too large/)
  })

  it('rejects memory-unrealistic powers of two before twiddle allocation', () => {
    const data = new Float64Array(8)
    expect(() => fft(data, 2 ** 21)).toThrow(/too large/)
  })
...
describe('fftNd / ifftNd input validation', () => {
  ...
  it('fftNd rejects oversized axes before coercing them into Uint32 WASM payloads', () => {
    const data = new Float64Array(8)
    expect(() => fftNd(data, [2 ** 31])).toThrow(/dimension too large/)
  })

  it('fftNd rejects unsafe total-site products before buffer-length comparison', () => {
    const data = new Float64Array(8)
    expect(() => fftNd(data, [2 ** 20, 2 ** 20])).toThrow(/total sites too large/)
  })
})
