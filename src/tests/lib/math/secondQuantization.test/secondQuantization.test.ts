...
describe('coherentFockCoefficients', () => {
  ...
  it('normalizes malformed maxN and non-finite alpha inputs', () => {
    expect(coherentFockCoefficients(1, 0, Number.NaN)).toEqual([])
    expect(coherentFockCoefficients(1, 0, -5)).toEqual([])
    expect(coherentFockCoefficients(1, 0, 3.9)).toHaveLength(3)
    expect(coherentFockCoefficients(1, 0, 1_000_000)).toHaveLength(FOCK_MAX_SAFE_LENGTH)

    const coeffs = coherentFockCoefficients(Number.NaN, Number.POSITIVE_INFINITY, 4)
    expect(coeffs).toHaveLength(4)
    expect(coeffs[0]!.re).toBe(1)
...
describe('squeezedFockCoefficients', () => {
  ...
  it('normalizes malformed maxN and non-finite squeeze inputs', () => {
    expect(squeezedFockCoefficients(0.5, 0, Number.NaN)).toEqual([])
    expect(squeezedFockCoefficients(0.5, 0, -4)).toEqual([])
    expect(squeezedFockCoefficients(0.5, 0, 5.9)).toHaveLength(5)
    expect(squeezedFockCoefficients(0.5, 0, 1_000_000)).toHaveLength(FOCK_MAX_SAFE_LENGTH)

    const vacuum = squeezedFockCoefficients(Number.NaN, Number.POSITIVE_INFINITY, 4)
    expect(vacuum[0]!.re).toBe(1)
    for (let i = 1; i < vacuum.length; i++) {
      expect(vacuum[i]!.re).toBe(0)
      expect(vacuum[i]!.im).toBe(0)
    }

    const huge = squeezedFockCoefficients(1000, 0, 200)
    for (const c of huge) {
      expect(Number.isFinite(c.re)).toBe(true)
      expect(Number.isFinite(c.im)).toBe(true)
    }
