...
describe('lnFactorial', () => {
  ...
  it('handles huge finite inputs with a bounded asymptotic path', () => {
    const n = 1_000_000_000
    const actual = lnFactorial(n)
    const leading = n * Math.log(n) - n + 0.5 * Math.log(2 * Math.PI * n)

    expect(Number.isFinite(actual)).toBe(true)
    expect(Math.abs((actual - leading) / leading)).toBeLessThan(1e-12)
    expect(lnFactorial(Number.MAX_VALUE)).toBe(Infinity)
  })
