...
describe('Vector Operations', () => {
  ...
  describe('magnitude', () => {
    ...
    it('keeps large finite vectors finite instead of overflowing squared sums', () => {
      const mag = magnitude([1e308, 1e308])
      expect(Number.isFinite(mag)).toBe(true)
      expect(mag / 1e308).toBeCloseTo(Math.SQRT2, 10)
    })
  })
  ...
  describe('normalize', () => {
    ...
    it('normalizes large finite vectors without overflow collapse', () => {
      const normalized = normalize([1e308, 1e308])
      expect(normalized[0]).toBeCloseTo(1 / Math.SQRT2, 10)
      expect(normalized[1]).toBeCloseTo(1 / Math.SQRT2, 10)
      expect(magnitude(normalized)).toBeCloseTo(1, 10)
    })

    it('rejects non-finite vectors', () => {
      expect(() => normalize([Infinity, 1])).toThrow(/non-finite/)
      expect(() => normalize([Number.NaN, 1])).toThrow(/non-finite/)
    })
  })
