...
describe('computeJointReducedDensityMatrix', () => {
  it.each([
    { label: 'empty', dims: [] },
    { label: 'duplicate', dims: [0, 0] },
    { label: 'unsorted', dims: [1, 0] },
    { label: 'out-of-range', dims: [0, 2] },
    { label: 'non-integer', dims: [0, 1.5] },
  ])('returns null for invalid kept dimensions: $label', ({ dims }) => {
    const re = new Float32Array(4)
    const im = new Float32Array(4)

    expect(computeJointReducedDensityMatrix(re, im, [2, 2], dims)).toBeNull()
  })
