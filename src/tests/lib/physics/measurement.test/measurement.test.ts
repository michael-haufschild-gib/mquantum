...
describe('sampleFromDensity', () => {
  ...
  it('rejects mismatched wavefunction buffers and non-finite amplitudes', () => {
    expect(() => sampleFromDensity(new Float32Array(3), new Float32Array(4), [4], [1])).toThrow(
      /psiRe length/
    )
    const psiRe = new Float32Array([1, Number.NaN, 0, 0])
    const psiIm = new Float32Array(4)
    expect(() => sampleFromDensity(psiRe, psiIm, [4], [1])).toThrow(/non-finite/)
  })

  it('rejects invalid sampling geometry before CDF construction', () => {
    expect(() => sampleFromDensity(new Float32Array(4), new Float32Array(4), [2, 2], [1])).toThrow(
      /spacing length/
    )
    expect(() =>
      sampleFromDensity(new Float32Array(4), new Float32Array(4), [2, 0], [1, 1])
    ).toThrow(/gridSize\[1\]/)
    expect(() =>
      sampleFromDensity(new Float32Array(4), new Float32Array(4), [2 ** 20, 2], [1, 1])
    ).toThrow(/grid product/)
  })
...
describe('sampleFromMarginalDensity', () => {
  ...
  it('rejects invalid marginal axes and non-finite amplitudes', () => {
    expect(() =>
      sampleFromMarginalDensity(new Float32Array(4), new Float32Array(4), [4], [1], 1)
    ).toThrow(/axis/)
    const psiRe = new Float32Array([1, 0, 0, 0])
    const psiIm = new Float32Array([0, Number.POSITIVE_INFINITY, 0, 0])
    expect(() => sampleFromMarginalDensity(psiRe, psiIm, [4], [1], 0)).toThrow(/non-finite/)
  })
})
...
describe('computeFullCollapse', () => {
  ...
  it('rejects inconsistent full-collapse geometry and non-finite centers', () => {
    expect(() => computeFullCollapse(5, [2, 2], [1, 1], [0, 0], 1)).toThrow(/grid product/)
    expect(() => computeFullCollapse(2 ** 21, [2 ** 20, 2], [1, 1], [0, 0], 1)).toThrow(
      /grid product/
    )
    expect(() => computeFullCollapse(4, [2, 2], [1], [0, 0], 1)).toThrow(/spacing length/)
    expect(() => computeFullCollapse(4, [2, 2], [1, 1], [0], 1)).toThrow(/vector length/)
    expect(() => computeFullCollapse(4, [2, 2], [1, 1], [0, Number.NaN], 1)).toThrow(/finite/)
  })

  it('rejects non-finite or negative full-collapse sigma', () => {
    expect(() => computeFullCollapse(4, [4], [1], [0], Number.NaN)).toThrow(/sigma/)
    expect(() => computeFullCollapse(4, [4], [1], [0], -1)).toThrow(/sigma/)
  })
...
describe('computePartialCollapse', () => {
  ...
  it('rejects malformed partial-collapse inputs before applying the envelope', () => {
    expect(() =>
      computePartialCollapse(new Float32Array(3), new Float32Array(4), [4], [1], 0, 0, 1)
    ).toThrow(/psiRe length/)
    expect(() =>
      computePartialCollapse(
        new Float32Array(4),
        new Float32Array(4),
    ...
    expect(() =>
      computePartialCollapse(new Float32Array(4), new Float32Array(4), [4], [1], 0, Number.NaN, 1)
    ).toThrow(/axisPosition/)
    expect(() =>
      computePartialCollapse(new Float32Array(4), new Float32Array(4), [4], [1], 0, 0, -1)
    ).toThrow(/sigma/)
    const psiRe = new Float32Array([1, 2, Number.NaN, 4])
    expect(() => computePartialCollapse(psiRe, new Float32Array(4), [4], [1], 0, 0, 1)).toThrow(
      /non-finite/
    )
  })
...
describe('normalizeWavefunctionInSamplingMeasure', () => {
  ...
  it('rejects invalid normalization buffers and non-finite amplitudes', () => {
    expect(() =>
      normalizeWavefunctionInSamplingMeasure(new Float32Array(3), new Float32Array(4), [4], [1])
    ).toThrow(/psiRe length/)
    expect(() =>
      normalizeWavefunctionInSamplingMeasure(
