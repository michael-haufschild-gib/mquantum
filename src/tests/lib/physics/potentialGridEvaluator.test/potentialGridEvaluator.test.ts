...
describe('evaluatePotentialGrid', () => {
  ...
  describe('edge cases', () => {
    it('rejects invalid grid geometry before allocation', () => {
      const evaluator = parse('0')

      expect(() => evaluatePotentialGrid(evaluator, [], [])).toThrow(/at least one axis/)
      expect(() => evaluatePotentialGrid(evaluator, [0], [1])).toThrow(/positive safe integer/)
      expect(() => evaluatePotentialGrid(evaluator, [2], [])).toThrow(/spacing length/)
      expect(() => evaluatePotentialGrid(evaluator, [2], [Number.NaN])).toThrow(
        /finite and positive/
      )
      expect(() => evaluatePotentialGrid(evaluator, [2 ** 32, 2 ** 22], [1, 1])).toThrow(
        /site budget/
      )
      expect(() => evaluatePotentialGrid(evaluator, [2 ** 20, 2], [1, 1])).toThrow(/site budget/)
    })
    ...
    it('finite JS values outside f32 range are replaced with 0 before GPU upload', () => {
      const V = evaluatePotentialGrid(() => 1e100, [2], [1.0])
