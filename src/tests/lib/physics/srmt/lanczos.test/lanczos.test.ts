...
describe('lanczosTopK — edge cases', () => {
  ...
  it('rejects dense matrix sizes that would overflow n² bounds', () => {
    expect(() => lanczosTopK(new Float32Array(0), 1025, 2)).toThrow(/n²/)
  })

  it('rejects oversized callback Krylov basis before allocation', () => {
    expect(() =>
      lanczosTopKOp(
        (_x, y) => {
          y.fill(0)
    ...
    ).toThrow(/Krylov basis size/)
  })

  it('rejects buffer shorter than n²', () => {
    const A = new Float32Array(10)
    expect(() => lanczosTopK(A, 4, 2)).toThrow(/buffer length 10 < n² = 16/)
  })

  it('rejects non-finite dense matrix entries before iteration', () => {
    const A = new Float32Array(16)
    A[5] = Number.NaN
    expect(() => lanczosTopK(A, 4, 2)).toThrow(/non-finite matrix entry at index 5/)
  })

  it('rejects non-finite callback operator output', () => {
    expect(() =>
      lanczosTopKOp(
        (_x, y) => {
          y.fill(0)
          y[2] = Number.POSITIVE_INFINITY
        },
        4,
        2,
        1
      )
    ).toThrow(/non-finite operator output at index 2/)
  })
})
