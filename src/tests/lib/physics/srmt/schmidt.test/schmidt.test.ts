...
import { MAX_COMPLEX_SVD_MATRIX_ENTRIES } from '@/lib/physics/srmt/svd'
...
describe('schmidt.reshapeForClock', () => {
  ...
  it('rejects invalid and over-budget tensor shapes before reshape allocation', () => {
    expect(() => reshapeForClock({ chi: new Float32Array(0), gridSize: [4, 0, 4] }, 'a')).toThrow(
      /positive safe integer/
    )
    expect(() =>
      reshapeForClock(
        { chi: new Float32Array(0), gridSize: [MAX_COMPLEX_SVD_MATRIX_ENTRIES + 1, 1, 1] },
        'a'
      )
    ).toThrow(/Schmidt matrix entries/)
  })
})
