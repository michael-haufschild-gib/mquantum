...
import { complexSvdSingularValues, MAX_COMPLEX_SVD_MATRIX_ENTRIES } from '@/lib/physics/srmt/svd'
...
describe('complexSvdSingularValues', () => {
  it('rejects over-budget matrix and real-embedding sizes before scratch allocation', () => {
    expect(() =>
      complexSvdSingularValues({
        rows: MAX_COMPLEX_SVD_MATRIX_ENTRIES + 1,
        cols: 1,
        re: new Float64Array(0),
        im: new Float64Array(0),
      })
    ).toThrow(/matrix entries/)
