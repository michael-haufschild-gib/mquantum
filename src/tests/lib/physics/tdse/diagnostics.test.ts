...
describe('normDriftFromHistory', () => {
  ...
  it('returns 0 for single-entry history', () => {
    const history: TdseDiagnosticsSnapshot[] = [
      { simTime: 0, totalNorm: 1.0, maxDensity: 0.5, normDrift: 0 },
    ]
  ...
  it('computes correct fractional drift for increasing norm', () => {
    const history: TdseDiagnosticsSnapshot[] = [
      { simTime: 0, totalNorm: 1.0, maxDensity: 0.5, normDrift: 0 },
      { simTime: 1, totalNorm: 1.05, maxDensity: 0.5, normDrift: 0.05 },
    ]
  ...
  it('computes correct fractional drift for decreasing norm (absorber)', () => {
    const history: TdseDiagnosticsSnapshot[] = [
      { simTime: 0, totalNorm: 1.0, maxDensity: 0.5, normDrift: 0 },
      { simTime: 0.5, totalNorm: 0.98, maxDensity: 0.45, normDrift: -0.02 },
      { simTime: 1, totalNorm: 0.95, maxDensity: 0.4, normDrift: -0.05 },
    ]
    expect(normDriftFromHistory(history)).toBeCloseTo(-0.05)
  })
  ...
  it('returns 0 when initial norm is 0', () => {
    const history: TdseDiagnosticsSnapshot[] = [
      { simTime: 0, totalNorm: 0, maxDensity: 0, normDrift: 0 },
      { simTime: 1, totalNorm: 0.5, maxDensity: 0.3, normDrift: 0 },
    ]
    expect(normDriftFromHistory(history)).toBe(0)
  })
})
...
describe('TdseDiagnosticsHistory', () => {
  ...
  it('stores and retrieves snapshots', () => {
    ...
    h.push({ simTime: 0.1, totalNorm: 0.99, maxDensity: 0.48, normDrift: -0.01 })
  ...
  it('evicts oldest entries at capacity', () => {
    ...
    h.push({ simTime: 0, totalNorm: 1.0, maxDensity: 0.5, normDrift: 0 })
    h.push({ simTime: 1, totalNorm: 0.99, maxDensity: 0.49, normDrift: -0.01 })
    h.push({ simTime: 2, totalNorm: 0.98, maxDensity: 0.48, normDrift: -0.02 })
    h.push({ simTime: 3, totalNorm: 0.97, maxDensity: 0.47, normDrift: -0.03 })
  ...
  it('computes norm drift from history', () => {
    const h = new TdseDiagnosticsHistory()
    h.push({ simTime: 0, totalNorm: 1.0, maxDensity: 0.5, normDrift: 0 })
    h.push({ simTime: 1, totalNorm: 1.02, maxDensity: 0.5, normDrift: 0.02 })
