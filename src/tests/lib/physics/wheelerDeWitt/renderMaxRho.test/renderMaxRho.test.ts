...
describe('resolveWdwDensityGridSize', () => {
  it('rounds finite resolutions and preserves the exposed 256 upper bound', () => {
    expect(resolveWdwDensityGridSize(95.6)).toBe(96)
    expect(resolveWdwDensityGridSize(256)).toBe(256)
  })

  it('rejects non-finite or over-budget resolutions before density allocation', () => {
    expect(() => resolveWdwDensityGridSize(Number.NaN)).toThrow(/finite/)
    expect(() => resolveWdwDensityGridSize(257)).toThrow(/max supported size 256/)
  })
})
