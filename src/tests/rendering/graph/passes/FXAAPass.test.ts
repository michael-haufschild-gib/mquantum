...
describe('FXAAPass', () => {
  ...
  describe('execute', () => {
    it('should skip when size is invalid', () => {
      ...
      expect(() => fxaaPass.execute(mockContext as unknown as import('@/rendering/graph/types').RenderContext)).not.toThrow();
    });
  ...
  describe('inputs/outputs', () => {
    it('should declare correct input', () => {
      expect(fxaaPass.config.inputs).toEqual([
        { resourceId: 'sceneColor', access: 'read' },
      ]);
    });

    it('should declare correct output', () => {
      expect(fxaaPass.config.outputs).toEqual([
        { resourceId: 'antialiasedColor', access: 'write' },
      ]);
    });
  });
});
