 ...
 * the packed 1040-byte ArrayBuffer at named layout indices to confirm the writer
 *
...
describe('TDSE uniform pack — analog-Hawking island overlay', () => {
  it('struct size is 1040 bytes (CTC + island + metric rows + W6 overlay + invSpacing precompute)', () => {
    expect(TDSE_UNIFORM_SIZE).toBe(1040)
  })
  ...
  it('writes active island triple when overlay is enabled and radius is positive', () => {
    ...
    expect(packed.size).toBe(TDSE_UNIFORM_SIZE)
  })
