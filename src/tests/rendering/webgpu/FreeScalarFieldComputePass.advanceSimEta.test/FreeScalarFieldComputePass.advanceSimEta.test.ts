...
describe('computeFsfCflSubsteps', () => {
  ...
  it('returns the identity split for non-finite dt before using stiffness inputs', () => {
    const config = {
      ...DEFAULT_FREE_SCALAR_CONFIG,
      latticeDim: 3,
      gridSize: [64, 64, 64],
    ...
    expect(computeFsfCflSubsteps(config, Number.POSITIVE_INFINITY, 1, new Set())).toBe(1)
  })

  it('bounds active dimensions before iterating grid and spacing arrays', () => {
    const config = {
      ...DEFAULT_FREE_SCALAR_CONFIG,
    ...
    const substeps = computeFsfCflSubsteps(config, 1, 1, warned)
