...
describe('free scalar canonical δφ integrator shader contracts', () => {
  ...
  it('writeGrid shader renders a bounded cosmological freeze-out strain field view', () => {
    ...
    expect(freeScalarWriteGridBlock).toContain('let freezeOutGate = 1.0 - clamp(')
    expect(freeScalarWriteGridBlock).toContain('let phaseSpaceBalance = 1.0 - clamp(')
    expect(freeScalarWriteGridBlock).toContain('let fluxStrain = clamp(')
    expect(freeScalarWriteGridBlock).toContain(
