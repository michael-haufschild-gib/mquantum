...
describe('assertSharedMemoryFFTLog2', () => {
  it('rejects powers of two that cannot be represented by WGSL u32 uniforms', () => {
    expect(() => assertPow2Log2(2 ** 40)).toThrow(/u32/)
  })
...
describe('computeStridesPadded', () => {
  ...
  it('rejects active axes and stride products that do not fit WGSL u32 uniforms', () => {
    expect(() => computeStridesPadded([2 ** 32], 1)).toThrow(/gridSize\[0\].*u32/)
    expect(() => computeStridesPadded([2, 2 ** 32], 2)).toThrow(/gridSize\[1\].*u32/)
    expect(() => computeStridesPadded([2, 2 ** 16, 2 ** 16], 3)).toThrow(/stride\[0\].*u32/)
  })
...
describe('FFT uniform packers', () => {
  it('packs one axis slot per active axis and direction', () => {
    const packed = packFFTAxisUniforms({ gridSize: [4, 8], latticeDim: 2 }, 32)
    expect(packed.byteLength).toBe(4 * FFT_UNIFORM_SIZE)

    const first = new DataView(packed, 0, FFT_UNIFORM_SIZE)
    expect(first.getUint32(0, true)).toBe(8)
    expect(first.getFloat32(4, true)).toBe(1)
    expect(first.getUint32(8, true)).toBe(32)
    expect(first.getUint32(16, true)).toBe(3)
  })

  it('rejects invalid lattice dimensions before iterating axes', () => {
    expect(() =>
      packFFTStageUniforms({ gridSize: [4], latticeDim: Number.POSITIVE_INFINITY }, 4)
    ).toThrow(/latticeDim/)
    expect(() => packFFTAxisUniforms({ gridSize: [4], latticeDim: 0 }, 4)).toThrow(/latticeDim/)
  })

  it('rejects missing axes and invalid total site counts', () => {
    expect(() => packFFTAxisUniforms({ gridSize: [4], latticeDim: 2 }, 4)).toThrow(/gridSize\[1\]/)
    expect(() => packFFTStageUniforms({ gridSize: [4], latticeDim: 1 }, Number.NaN)).toThrow(
      /totalSites/
    )
  })
})
