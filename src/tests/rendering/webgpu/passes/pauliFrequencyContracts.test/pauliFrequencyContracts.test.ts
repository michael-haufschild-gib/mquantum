...
const pauliComputePass = readFileSync(
  resolve(root, 'src/rendering/webgpu/passes/PauliComputePass/index.ts'),
  'utf8'
)
const pauliUniformStaging = readFileSync(
  resolve(root, 'src/rendering/webgpu/passes/PauliComputePassUniformStaging/index.ts'),
  'utf8'
)
const pauliPotentialHalf = readFileSync(
  resolve(
    root,
    'src/rendering/webgpu/shaders/schroedinger/compute/pauliPotentialHalf.wgsl/pauliPotentialHalf.wgsl.ts'
  ),
  'utf8'
)
const pauliKinetic = readFileSync(
  resolve(
    root,
    'src/rendering/webgpu/shaders/schroedinger/compute/pauliKinetic.wgsl/pauliKinetic.wgsl.ts'
  ),
  'utf8'
)
...
describe('Pauli spin frequency contracts', () => {
  ...
  it('reports Larmor frequency for H = sigma dot B, not half the Bloch rate', () => {
    ...
    expect(pauliComputePass).toContain('const larmorFrequency = (2 * safeFieldStrength) / safeHbar')
  })
