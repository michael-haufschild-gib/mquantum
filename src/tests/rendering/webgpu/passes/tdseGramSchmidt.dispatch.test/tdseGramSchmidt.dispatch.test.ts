...
function createDeviceWithWriteSnapshots() {
  const writeSnapshots: WriteSnapshot[] = []
  const device = {
    createBuffer: vi.fn((descriptor: GPUBufferDescriptor) => ({
      ...createMockBuffer(String(descriptor.label ?? 'buffer')),
      size: Number(descriptor.size),
      usage: Number(descriptor.usage),
    })),
    createBindGroup: vi.fn(() => createMockBindGroup()),
    queue: {
      writeBuffer: vi.fn((buffer: GPUBuffer, offset: number, data: AllowSharedBufferSource) => {
        writeSnapshots.push({ buffer, offset, data: copyBufferSource(data) })
      }),
    },
...
function createRenderContext(device: GPUDevice, events: string[] = []): WebGPURenderContext {
  const encoder = createMockCommandEncoder()
  vi.mocked(encoder.copyBufferToBuffer).mockImplementation(
    (_src: GPUBuffer, srcOffset: GPUSize64) => {
      events.push(`copy:${srcOffset}`)
    }
  )
...
function createState(overrides: Partial<GramSchmidtState> = {}): GramSchmidtState {
  return {
    ...
    gsUniformStagingBuffer: null,
    gsUniformStagingByteSize: 0,
...
function readRecord(buffer: ArrayBuffer, recordIdx: number): ArrayBuffer {
  return buffer.slice(recordIdx * GS_UNIFORM_SIZE, (recordIdx + 1) * GS_UNIFORM_SIZE)
}
...
describe('dispatchGramSchmidt', () => {
  ...
  it('dispatches reduce, finalize, and subtract once per stored eigenstate', () => {
    ...
    const events: string[] = []
    const ctx = createRenderContext(device, events)
    ...
    expect(device.createBuffer).toHaveBeenCalledWith(
      expect.objectContaining({ label: 'gs-uniform-staging', size: 4 * GS_UNIFORM_SIZE })
    )
    expect(device.queue.writeBuffer).toHaveBeenCalledTimes(1)
    expect(writeSnapshots[0]?.offset).toBe(0)

    const stagedUniforms = writeSnapshots[0]!.data
    expect(readU32(readRecord(stagedUniforms, 0))).toEqual([513, 5, 0, 0])
    expect(readSubtractUniform(readRecord(stagedUniforms, 1))).toEqual({
      totalSites: 513,
      normSquared: 0.5,
    })
    expect(readU32(readRecord(stagedUniforms, 2))).toEqual([513, 5, 0, 0])
    expect(readSubtractUniform(readRecord(stagedUniforms, 3))).toEqual({
      totalSites: 513,
      normSquared: 2,
    })

    expect(vi.mocked(ctx.encoder.copyBufferToBuffer).mock.calls.map((call) => call[1])).toEqual([
      0, 16, 32, 48,
    ])
    expect(vi.mocked(ctx.encoder.copyBufferToBuffer).mock.calls.map((call) => call[3])).toEqual([
      0, 0, 0, 0,
    ])
    expect(vi.mocked(ctx.encoder.copyBufferToBuffer).mock.calls.map((call) => call[4])).toEqual([
      GS_UNIFORM_SIZE,
      GS_UNIFORM_SIZE,
      GS_UNIFORM_SIZE,
      GS_UNIFORM_SIZE,
