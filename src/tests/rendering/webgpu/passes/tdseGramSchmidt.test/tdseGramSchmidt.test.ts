...
describe('clearEigenstates', () => {
  it('destroys all eigenstate buffers and resets array', () => {
    ...
    const staging = createMockBuffer('gs-uniform-staging')
    const state = createGSState({
      gsUniformStagingBuffer: staging,
      gsUniformStagingByteSize: 128,
    })
    state.gsEigenstates = [
    ...
    expect(state.gsEigenstates).toHaveLength(0)
    expect(state.gsUniformStagingBuffer).toBeNull()
    expect(state.gsUniformStagingByteSize).toBe(0)
  })
