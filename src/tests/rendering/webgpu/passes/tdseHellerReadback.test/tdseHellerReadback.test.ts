...
import {
  applyHellerPerFrame,
  copyFiniteWavefunctionSnapshot,
...
describe('copyFiniteWavefunctionSnapshot', () => {
  it('deinterleaves finite wavefunctions and computes the anchor norm', () => {
    const snapshot = copyFiniteWavefunctionSnapshot(new Float32Array([1, 2, 3, 4]), 2)

    expect(snapshot?.re).toEqual(new Float32Array([1, 3]))
    expect(snapshot?.im).toEqual(new Float32Array([2, 4]))
    expect(snapshot?.norm).toBe(30)
  })

  it('rejects malformed or non-finite wavefunction anchors', () => {
    expect(copyFiniteWavefunctionSnapshot(new Float32Array([1, 0]), 2)).toBeNull()
    expect(copyFiniteWavefunctionSnapshot(new Float32Array([1, Number.NaN]), 1)).toBeNull()
    expect(copyFiniteWavefunctionSnapshot(new Float32Array([1, 0]), Number.NaN)).toBeNull()
  })
})
