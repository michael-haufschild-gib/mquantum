...
describe('computeStructLayout', () => {
  ...
  it('rejects duplicate field names', () => {
    ...
    ).toThrow(/Duplicate WGSL struct field name: value/)
  })

  it('rejects empty field names', () => {
    expect(() => computeStructLayout([{ name: '', type: 'f32' }] as const)).toThrow(
      /field name must not be empty/
    )
  })

  it('rejects invalid array counts', () => {
    expect(() => arr('f32', 0)).toThrow(/positive safe integer/)
    expect(() => arr('vec4f', 1.5)).toThrow(/positive safe integer/)
  })
})
...
describe('zeroReservedFields', () => {
  ...
  it('rejects undersized buffers before partial zeroing', () => {
    const layout = computeStructLayout([{ name: '_arr', type: arr('vec4f', 2) }] as const)
    const buf = new Float32Array(4)
