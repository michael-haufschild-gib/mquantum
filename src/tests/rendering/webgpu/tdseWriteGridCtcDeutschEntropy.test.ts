...
describe('tdseWriteGrid CTC Deutsch entropy field view', () => {
  it('documents ctcDeutschEntropy as fieldView enum 12 without moving existing CTC views', () => {
    expect(tdseUniformsBlock).toContain('10=ctcResidual, 11=ctcLoopGain, 12=ctcDeutschEntropy')
  })
  ...
  it('reuses guarded mirror sampling and zeroes empty mirror pairs', () => {
    ...
    expect(entropyStart).toBeGreaterThanOrEqual(0)
  ...
  it('implements balanced mirror-pair paradox entropy', () => {
    ...
    expect(entropyBody).toContain('let delta = atan2(sin(phaseMismatch), cos(phaseMismatch));')
    expect(entropyBody).toContain('let balance = 4.0 * density * mirror.density / (denom * denom);')
