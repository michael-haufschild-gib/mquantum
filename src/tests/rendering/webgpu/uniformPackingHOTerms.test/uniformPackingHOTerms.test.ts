...
describe('packPrecomputedHOTerms', () => {
  ...
  it('sanitizes non-finite time, energy, and coefficients before GPU-facing writes', () => {
    const { floatView, intView } = makeViews()
    intView[I.termCount] = 2
    floatView[I.energy] = Number.POSITIVE_INFINITY
    floatView[I.energy + 1] = 1
    floatView[I.coeff] = 1
    floatView[I.coeff + 1] = Number.NaN
    floatView[I.coeff + 4] = Number.POSITIVE_INFINITY
    floatView[I.coeff + 5] = 2

    packPrecomputedHOTerms(floatView, intView, Number.MAX_VALUE, Number.MAX_VALUE)

    for (let i = 0; i < 8; i++) {
      expect(Number.isFinite(floatView[I.precomputedTerm + i]!)).toBe(true)
    }
    // Non-finite time and energy collapse to zero phase; non-finite coeff parts collapse to zero.
    expect(floatView[I.precomputedTerm]).toBeCloseTo(1, 6)
    expect(floatView[I.precomputedTerm + 1]).toBeCloseTo(0, 6)
    expect(floatView[I.precomputedTerm + 4]).toBeCloseTo(0, 6)
    expect(floatView[I.precomputedTerm + 5]).toBeCloseTo(2, 6)
  })
})
