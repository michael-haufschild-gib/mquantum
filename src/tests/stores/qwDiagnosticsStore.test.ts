...
describe('qwDiagnosticsStore', () => {
  ...
  describe('initial state', () => {
    it('starts with no data and unit norm', () => {
      const s = useQwDiagnosticsStore.getState()
  ...
  describe('pushDiagnostics', () => {
    it('captures initial norm on first call', () => {
      useQwDiagnosticsStore.getState().pushDiagnostics(0.98, 1, 0, 0)
      const s = useQwDiagnosticsStore.getState()
    ...
    it('preserves initial norm on subsequent calls', () => {
      useQwDiagnosticsStore.getState().pushDiagnostics(1.0, 1, 0, 0)
      useQwDiagnosticsStore.getState().pushDiagnostics(0.95, 2, 0, 0)
      expect(useQwDiagnosticsStore.getState().initialNorm).toBe(1.0)
    })

    it('computes norm drift correctly: (norm - norm0) / norm0', () => {
      // First call sets norm0 = 1.0
      useQwDiagnosticsStore.getState().pushDiagnostics(1.0, 1, 0, 0)
      expect(useQwDiagnosticsStore.getState().normDrift).toBeCloseTo(0)

      // Second call: norm = 0.95, drift = (0.95 - 1.0) / 1.0 = -0.05
      useQwDiagnosticsStore.getState().pushDiagnostics(0.95, 2, 0, 0)
      expect(useQwDiagnosticsStore.getState().normDrift).toBeCloseTo(-0.05)

      // Third call: norm = 1.05, drift = (1.05 - 1.0) / 1.0 = 0.05
      useQwDiagnosticsStore.getState().pushDiagnostics(1.05, 3, 0, 0)
      expect(useQwDiagnosticsStore.getState().normDrift).toBeCloseTo(0.05)
    })

    it('computes position mean correctly: posSum / totalNorm', () => {
      // totalNorm=2, posSum=6 -> mean=3
      useQwDiagnosticsStore.getState().pushDiagnostics(2.0, 1, 6.0, 20.0)
      expect(useQwDiagnosticsStore.getState().positionMean).toBeCloseTo(3.0)
    })
    ...
    it('computes variance correctly: posSqSum/norm - mean^2', () => {
      ...
      useQwDiagnosticsStore.getState().pushDiagnostics(4.0, 1, 8.0, 20.0)
      expect(useQwDiagnosticsStore.getState().positionVariance).toBeCloseTo(1.0)
    })
    ...
    it('clamps variance to zero (avoids negative from floating point)', () => {
      ...
      useQwDiagnosticsStore.getState().pushDiagnostics(1.0, 1, 1.0, 0.99)
      expect(useQwDiagnosticsStore.getState().positionVariance).toBe(0)
    })

    it('handles zero totalNorm gracefully (returns 0 for mean and variance)', () => {
      useQwDiagnosticsStore.getState().pushDiagnostics(0, 5, 10, 100)
      const s = useQwDiagnosticsStore.getState()
      expect(s.positionMean).toBe(0)
      expect(s.positionVariance).toBe(0)
    })
    ...
    it('handles zero initial norm gracefully (normDrift = 0)', () => {
      ...
      useQwDiagnosticsStore.getState().pushDiagnostics(0, 1, 0, 0)
      expect(useQwDiagnosticsStore.getState().normDrift).toBe(0)

      // Second call still zero norm0, drift should be 0
      useQwDiagnosticsStore.getState().pushDiagnostics(0.5, 2, 0, 0)
      expect(useQwDiagnosticsStore.getState().normDrift).toBe(0)
    })

    it('tracks step count correctly', () => {
      useQwDiagnosticsStore.getState().pushDiagnostics(1.0, 42, 0, 0)
      expect(useQwDiagnosticsStore.getState().stepCount).toBe(42)

      useQwDiagnosticsStore.getState().pushDiagnostics(1.0, 100, 0, 0)
      expect(useQwDiagnosticsStore.getState().stepCount).toBe(100)
    })

    it('updates totalNorm on every call', () => {
      useQwDiagnosticsStore.getState().pushDiagnostics(1.0, 1, 0, 0)
      expect(useQwDiagnosticsStore.getState().totalNorm).toBe(1.0)

      useQwDiagnosticsStore.getState().pushDiagnostics(0.87, 2, 0, 0)
      expect(useQwDiagnosticsStore.getState().totalNorm).toBe(0.87)
    })
    ...
    it('realistic quantum walk scenario: ballistic spreading', () => {
      ...
      useQwDiagnosticsStore.getState().pushDiagnostics(1.0, 0, 0, 0)
      expect(useQwDiagnosticsStore.getState().positionVariance).toBe(0)

      // Step 10: spread out, norm preserved
      // mean = posSum/norm = 5/1 = 5, variance = posSqSum/norm - mean^2 = 50/1 - 25 = 25
      useQwDiagnosticsStore.getState().pushDiagnostics(1.0, 10, 5.0, 50.0)
      expect(useQwDiagnosticsStore.getState().positionVariance).toBeCloseTo(25.0)
      expect(useQwDiagnosticsStore.getState().normDrift).toBeCloseTo(0)

      // Step 20: more spread, slight norm loss
      // mean = 10/0.99 ≈ 10.1, variance = 200/0.99 - 10.1^2 ≈ 202.02 - 102.01 ≈ 100.01
      useQwDiagnosticsStore.getState().pushDiagnostics(0.99, 20, 10.0, 200.0)
      expect(useQwDiagnosticsStore.getState().normDrift).toBeCloseTo(-0.01)
      expect(useQwDiagnosticsStore.getState().positionVariance).toBeGreaterThan(0)
    })
  })

  describe('reset', () => {
    it('restores all fields to initial defaults after mutations', () => {
      useQwDiagnosticsStore.getState().pushDiagnostics(0.95, 50, 25.0, 1000.0)
      expect(useQwDiagnosticsStore.getState().hasData).toBe(true)
    ...
    it('allows fresh initial norm capture after reset', () => {
      ...
      useQwDiagnosticsStore.getState().reset()
      useQwDiagnosticsStore.getState().pushDiagnostics(0.5, 1, 0, 0)
      expect(useQwDiagnosticsStore.getState().initialNorm).toBe(0.5)
    })
  })
  ...
  describe('rapid pushDiagnostics calls', () => {
    it('handles 100 rapid calls without state corruption', () => {
      for (let i = 0; i < 100; i++) {
        ...
        useQwDiagnosticsStore.getState().pushDiagnostics(norm, i, posSum, posSqSum)
      }
